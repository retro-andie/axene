/*
** Addition_Cadre.c for Xclamation in Page/
**
...
**
** Started on  Mon Jul 17 20:30:05 1995 Emmanuel Paris
** Last update Thu Oct 24 16:17:50 1996 Emmanuel Paris
*/


#include "Page.h"

void addition_cadre(cpage)
   c_Page	*cpage;
{
  l_Cadre	*lcadre;
  l_Cadre	*svglcadre;
  ...
  if (max_type > CADRE_EMPTY && maxcadre != sortedlcadre)
    {
      F(svglcadre->cadre).detachobject(svglcadre->cadre);
      svglcadre->cadre->object = maxcadre->cadre->object;
      svglcadre->cadre->object_type = maxcadre->cadre->object_type;
      maxcadre->cadre->object = NULL;
      maxcadre->cadre->object_type = CADRE_EMPTY;
      angle = maxcadre->cadre->sAngle;
    }
  ...
  while(lcadre)
    {
      lpoint=(l_Point *)AdditionCadre(cadre,lcadre->cadre);
      if (!lpoint)
	{
	  while(lcadre)
	    {
