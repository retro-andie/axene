/*
** Drag_Cadre.c for Xclamation in Page/
**
...
** Last update Fri Jan  7 15:59:03 2000 Emmanuel Paris
*/
...
d_Drag_Cadre *drag_cadre_init(display,window,lcadre,cursor,function,xo,yo)
Display		*display;
Window		window;
l_Cadre		*lcadre;
c_Cursor	*cursor;
...
void drag_cadre_done(drag,window_source,data_source,window_target,data_target)
d_Drag_Cadre	*drag;
...
void drag_cadre_move_done(cpage,listcadre,xo,yo)
c_Page	*cpage;
l_Cadre	*listcadre;
int	xo,yo;
{
  ...
  while(lcadre)
    {
      ...
      if(((color = F(color).virtualCopy(color, cpage->BaseStd)) == NULL) ||
	 ...
	 !F(frame).setBackgroundColor(frame, color)) continue;
      ...
      frame->Forme->Orig_X = frame->sOrig_X / cpage->sScale-xo;
      frame->Forme->Orig_Y = frame->sOrig_Y / cpage->sScale-yo;
      frame->sOrig_X = frame->Forme->Orig_X * cpage->sScale;
      frame->sOrig_Y = frame->Forme->Orig_Y * cpage->sScale;
      frame->sScale = cpage->sScale;
      frame->sFactor = cpage->sFactor;
      ...
      if(frame->object_type != CADRE_EMPTY)
	{
	  ...
	  if((CHANGE_BASE(ptr)(ptr, cpage->BaseStd) != XC_NO_ERROR) ||
      ...
      if(F(cpage).add_cadre(cpage, name, priority++, frame) ==
	 XC_NAME_ALREADY_EXIST)
	{
	  name2 = F(cpage).copy_cadre_name(cpage, name);
	  Xc_free(name);
	  name = name2;
	  F(cpage).add_cadre(cpage, name, priority++, frame);
	}
      lcadre2 = F(cpage).get_cadre_by_name(cpage, name);
      Xc_free(name);
      F(lcadre2->cadre).reinit_X_info(lcadre2->cadre, cpage->X_info.display,
				      cpage->X_info.window);

      F(cpage).draw_cadre(cpage, lcadre2);
      F(cpage).select_cadre(cpage, lcadre2, FALSE);
...
void drag_cadre_copy_done(cpage,listcadre,xo,yo)
c_Page	*cpage;
l_Cadre	*listcadre;
int	xo,yo;
{
  ...
  while(lcadre)
    {
      cadre=(c_Cadre *)COPY(c_Cadre)(lcadre->cadre,cpage->BaseStd);
      cadre->Forme->Orig_X=cadre->sOrig_X/cpage->sScale-xo;
      cadre->Forme->Orig_Y=cadre->sOrig_Y/cpage->sScale-yo;
      cadre->sOrig_X=cadre->Forme->Orig_X*cpage->sScale;
      cadre->sOrig_Y=cadre->Forme->Orig_Y*cpage->sScale;
