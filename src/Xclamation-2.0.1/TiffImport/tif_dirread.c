...
static	void EstimateStripByteCounts ___PROTO((TIFF*, TIFFDirEntry*, uint16));
static	void MissingRequired ___PROTO((TIFF*,  char*));
static	int CheckDirCount ___PROTO((TIFF*, TIFFDirEntry*, uint32));
static	tsize_t TIFFFetchData ___PROTO((TIFF*, TIFFDirEntry*, char*));
static	tsize_t TIFFFetchString ___PROTO((TIFF*, TIFFDirEntry*, char*));
static	float TIFFFetchRational ___PROTO((TIFF*, TIFFDirEntry*));
static	int TIFFFetchNormalTag ___PROTO((TIFF*, TIFFDirEntry*));
static	int TIFFFetchPerSampleShorts ___PROTO((TIFF*, TIFFDirEntry*, int*));
static	int TIFFFetchShortArray ___PROTO((TIFF*, TIFFDirEntry*, uint16*));
static	int TIFFFetchStripThing ___PROTO((TIFF*, TIFFDirEntry*, long, uint32**));
static	int TIFFFetchExtraSamples ___PROTO((TIFF*, TIFFDirEntry*));
static	int TIFFFetchRefBlackWhite ___PROTO((TIFF*, TIFFDirEntry*));
static	int TIFFFetchJPEGQTables ___PROTO((TIFF*, TIFFDirEntry*));
static	int TIFFFetchJPEGCTables ___PROTO((TIFF*, TIFFDirEntry*, u_char***));
static	float TIFFFetchFloat ___PROTO((TIFF*, TIFFDirEntry*));
static	int TIFFFetchFloatArray ___PROTO((TIFF*, TIFFDirEntry*, float*));
static	int TIFFFetchShortPair ___PROTO((TIFF*, TIFFDirEntry*));
#ifdef STRIPCHOP_SUPPORT
static	void ChopUpSingleUncompressedStrip ___PROTO((TIFF*));
#endif
 ...
 */
 ...
 } else 
 {
  ...
  if ((int)(off + sizeof (short)) > tif->tif_size) 
  {
   TIFFError(tif->tif_name,
	     "Can not read TIFF directory count");
   return (0);
  ...
  if (dir == NULL)
   return (0);
  if ((int)(off + dircount*sizeof (TIFFDirEntry)) > tif->tif_size) 
  {
   TIFFError(tif->tif_name, "Can not read TIFF directory");
   goto bad;
  } else
  ...
  if ((int)(off + sizeof (uint32)) < tif->tif_size)
 ...
 for (dp = dir, n = dircount; n > 0; n--, dp++) 
 {
   ...
   */
   ...
   if (cp != NULL) 
   {
     ...
     */
     if ((long)dp->tdir_count == (1L<<td->td_bitspersample))
      v = 0;
...
uint16 dircount;
{
  ...
  */ 
  if ((toff_t)(td->td_stripoffset[0] + td->td_stripbytecount[0]) > filesize)
...
char* cp;
{
 ...
 {
  if ((int)(dir->tdir_offset + cc) > tif->tif_size)
   goto bad;
 ...
 */
static int TIFFFetchRefBlackWhite(tif, dir)
TIFF* tif;
TIFFDirEntry* dir;
{
  ...
  if ((ok = (cp && TIFFFetchLongArray(tif, dir, (uint32*)cp))) != 0)
  {
   ...
   {
    int i;
    for (i = 0; i < (int)dir->tdir_count; i++)
