#ifndef lint
static char rcsid[] = "$Header: /usr/people/sam/tiff/libtiff/RCS/tif_read.c,v 1.55 93/08/26 14:26:43 sam Exp $";
#endif
...
static	int TIFFFillStrip ___PROTO((TIFF*, tstrip_t));
static	int TIFFFillTile ___PROTO((TIFF*, ttile_t));
static	int TIFFStartStrip ___PROTO((TIFF*, tstrip_t));
static	int TIFFStartTile ___PROTO((TIFF*, ttile_t));
static	int TIFFCheckRead ___PROTO((TIFF*, int));
...
char* module;
{
 ...
 {
  if ((int)(td->td_stripoffset[strip] + size) > tif->tif_size)
  {
   TIFFError(module,
	     "%s: Seek error at scanline %lu, strip %lu",
	     tif->tif_name, (u_long) tif->tif_row, (u_long) strip);
...
tstrip_t strip;
{
 ...
 bytecount = td->td_stripbytecount[strip];
 if (isMapped(tif) && (td->td_fillorder == tif->tif_fillorder ||
  ...
  */
  ...
  if ((int)(td->td_stripoffset[strip] + bytecount) > tif->tif_size)
  {
   /*
...
char* module;
{
 ...
 {
  if ((int)(td->td_stripoffset[tile] + size) > tif->tif_size) 
  {
   TIFFError(module,
	     "%s: Seek error at row %ld, col %ld, tile %ld",
	     tif->tif_name,
...
ttile_t tile;
{
  ...
  */
  if ((tif->tif_flags & TIFF_MYBUFFER) && tif->tif_rawdata)
   _TIFFfree(tif->tif_rawdata);
  tif->tif_flags &= ~TIFF_MYBUFFER;
  if ((int)(td->td_stripoffset[tile] + bytecount) > tif->tif_size)
  {
