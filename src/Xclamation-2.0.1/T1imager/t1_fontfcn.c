...
char *t1_strerror[] = { "No error",
  "Unexpected end of file", "Scan error", "Out of memory",
  "File open error", "True", "False", "End"};
...
char *env;
{
  ...
  vm_free = vm_size - ( vm_next - t1imager_vm_base);
  FontP->Subrs.len = 0;
...
#if 0
  /*--- Arrrg! Should get the FontMatrix! - Nucleus ---*/
    ...
    switch (dictP[N].value.type) {
         ...
         if (strcmp(infoName,"FontMatrix") == 0) {
           ...
           for (i=0;i<6;i++) {
             if (valueP->type == OBJ_INTEGER )
               ((real_t *)infoValue)[i] = valueP->data.integer;
             else
               ((real_t *)infoValue)[i] = valueP->data.real;
            valueP++;
           }
         }
       ...
       case OBJ_BOOLEAN:
         *((int *)infoValue) = dictP[N].value.data.integer;
         break;
       case OBJ_REAL:
         *((real_t *)infoValue) = dictP[N].value.data.real;
         break;
       case OBJ_NAME:
       case OBJ_STRING:
