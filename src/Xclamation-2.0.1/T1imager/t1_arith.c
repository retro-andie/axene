...
being a sequence of digits.  We'll take a digit to be a 'int16_t'.
The following assumption must be valid for these algorithms to work:
:ol.
:li.A 'int32_t' is two 'int16_t's.
:eol.
The following code is INDEPENDENT of:
:ol.
:li.The actual size of a int16_t.
:li.Whether shorts and longs are stored most significant byte
first or least significant byte first.
:eol.

SHORTSIZE is the number of bits in a int16_t; LONGSIZE is the number of
bits in a int32_t; MAXSHORT is the maximum uint16_t:
*/
/*SHARED LINE(S) ORIGINATED HERE*/
/*
  ASSEMBLE concatenates two shorts to form a int32_t:
  */
#define     ASSEMBLE(hi,lo)   ((((uint32_t)hi)<<SHORTSIZE)+(lo))
/*
  HIGHDIGIT extracts the most significant int16_t from a int32_t; LOWDIGIT
  extracts the least significant int16_t from a int32_t:
  */
#define     HIGHDIGIT(u)      ((u)>>SHORTSIZE)
#define     LOWDIGIT(u)       ((u)&MAXSHORT)

/*
  SIGNBITON tests the high order bit of a int32_t 'w':
  */
 ...
 * 'int16_t'.  It is worthwhile making this fast:
 */
 ...
 /*
 * At this point the divisor is a true 'int32_t' so we must use
 * Knuth's algorithm.
 *
 ...
 */
 ...
 if ((u1u2 >> (LONGSIZE - shift)) != 0 && shift != 0)
  Xc_FATAL(("DLdiv:  dividend too large"));
 ...
 */
 qhat = ((int)HIGHDIGIT(u1u2) == v1) ? MAXSHORT : (int)(u1u2 / v1);
 /*
 ...
 */
 u1u2 = t;
 if (HIGHDIGIT(u1u2) != 0)
  Xc_FATAL(("divide algorithm error"));
