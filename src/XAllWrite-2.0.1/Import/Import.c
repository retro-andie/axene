/*
** Import.c for XAllWrite in Import/
** Object to import various text file format
**
...
** Last update Mon Aug 10 14:37:56 1998 Emmanuel Paris
*/
...
static void import ___PROTO((c_Import *This, c_Document *current_document));
...
static void import(This, current_document)
c_Import	*This;
c_Document	*current_document;
{
 ...
 {
  if (CreateImportDocumentSelector(wMainWindow, This, current_document!=NULL) 
      == FALSE)
   return;
 }
 else
 {
  ManageImportDocumentSelector(current_document!=NULL);
 }
 ...
 if (!current_document || 
     IMPORT_CREATE_NEW_DOCUMENT(This->import_format[This->format_selected])
     (This->import_format[This->format_selected]) == TRUE)
 {
  new_document = TRUE;
  document = (c_Document *)NEW(c_Document)();
 }
 else
 {
  new_document = FALSE;
 ...
 load_error = IMPORT_LOAD(This->import_format[This->format_selected])
  (This->import_format[This->format_selected], document);
 ...
 if (new_document)
 {
  ...
  F(document).Init_Document(document,
  ...
  /*    {
	...
	ptr = document->BaseStd.cell_style_base->cell_style;
	while(ptr)
	{
	printf("cs: %p count: %d\n", ptr, ptr->count);
  ...
  XStoreName(XtDisplay(WIDGET(MainDocument)), document->window, doc_name);
  F(DocList).Add(DocList, document, DOCUMENT_WORDP, doc_name);
  Xc_free(doc_name);
  Xc_TRACE(("manage"));
  F(Gwm).ManageWin(Gwm, document->display, document->window,
