...
*/
#include "t1_port.h"
#include "t1_objects.h"	/* get #define for abort() */
...
static int16_t uncombined = 0; /* current number of uncombined blocks          */
...
register long *addr;           /* user's memory to be returned   */
{
  ...
  /*
     ...
     */
  if (size >= 0)
    Xc_FATAL(("free: bad size"));
  if (addr[-1 - size] != size)
    Xc_FATAL(("free: mismatched size"));
  /*
  ...
  if (++uncombined > MAXUNCOMBINED) {
    combine();
    if (mallocdebug) {
      printf("xiFree(%8p) with combine, ", addr);
      dumpchain();
    }
  }
  else {
    if (mallocdebug) {
      printf("xiFree(%8p), ", addr);
      dumpchain();
    }
  }
...
static void combine()
{
  ...
  if (p == &firstfree)
    Xc_FATAL(("why are we combining?"));
  
  addr = (long *) p;
  size = - p->size;
  if (--uncombined < 0)
    Xc_FATAL(("too many combine()s"));
...
unsigned Size;       /* number of bytes the user requested           */
{
  ...
  if (size < (long)(sizeof(struct freeblock) + sizeof(long)))
  ...
  for (p=firstfree.fore; p != firstcombined; p=p->fore) {
    if (p->size == size) {
      ...
      uncombined--;
      if (mallocdebug) {
	printf("fast xiMalloc(%ld) = %8p, ", size, p);
	dumpchain();
      }
  ...
  if (mallocdebug) {
    printf("slow xiMalloc(%ld) @ %8p, ", size, area);
    dumpchain();
  }
...
static void dumpchain()
{
  ...
  for (p = firstfree.fore, i=uncombined; p != firstcombined;
       p = p->fore) {
    if (--i < 0)
      Xc_FATAL(("too many uncombined areas"));
    size = p->size;
    printf(". . . area @ %8p, size = %ld\n", p, -size);
    if (size >= 0 || size != ((int *) p)[-1 - size])
      Xc_FATAL(("dumpchain: bad size"));
    if (p->back != back)
      Xc_FATAL(("dumpchain: bad back"));
    back = p;
  }
  ...
  if (back != lastfree.back)
    Xc_FATAL(("dumpchain: bad lastfree"));
}
  ...
  while (wholesize > 0) {
	...
	printf("\n");
      ...
      printf("Free %d bytes at %x\n", size * sizeof(long),
	     area);
      if (size == 0)
	Xc_FATAL(("zero sized memory block"));
      
      for (p = firstfree.fore; p != NULL; p = p->fore)
	if ((long *) p == area) break;
      if ((long *) p != area)
	Xc_FATAL(("not found on forward chain"));
      
      for (p = lastfree.back; p != NULL; p = p->back)
	if ((long *) p == area) break;
      if ((long *) p != area)
	Xc_FATAL(("not found on backward chain"));
    }
    if (area[0] != area[size - 1])
      Xc_FATAL(("unmatched check sizes"));
    area += size;
    wholesize -= size;
  }
}
