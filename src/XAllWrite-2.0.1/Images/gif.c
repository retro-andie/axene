/*
** gif.c for Xclamation, XAllWrite and XMayday in Images/
** Loading gif routine
**
...
** Last update Thu Jan  6 16:54:17 2000 Emmanuel Paris
*/
...
static unsigned char *getSubBlocks ___PROTO((FILE *file));
static boolean skipSubBlocks ___PROTO((FILE *file));
static char *depackLZW ___PROTO((FILE *file, int width, int height));
static void bitfieldInit ___PROTO((FILE *file, bitfield_t *bitfield));
...
char	*name;
{
 ...
 if (fread(id, sizeof(char), 6, file) != 6)
 {
  ...
  *buffer = 0;
  ...
  while(fread(buffer, 1, 1, file) == 1)
   ...
   else if(*buffer == ',')
   {
    ...
    if (fread(buffer, 9, 1, file) != 1) break;
...
c_Image		*Image;
{
 ...
 if ( fread(data, sizeof(char) * MIN(filesize, Xc_GIF_HEADER_LENGTH),
	    1, file) != 1)
 {
  Xc_WARNING(("Error while reading file %s",Image->filename));
  fclose(file);
...
FILE *file;
{
 ...
 while(fread(&len, 1, 1, file) == 1)
 {
  ...
  if((new = Xc_realloc("old", old, total + len)) == NULL)
   break;
  old = new;
  if(fread(old + total, (long)len, 1, file) != 1)
   break;
  total += len;
 }
...
FILE *file;
{
 ...
 while(fread(&len, 1, 1, file) == 1)
 {
  ...
  pos += len;
  if(fseek(file, (long)len, SEEK_CUR) != pos)
   break;
 }
 return FALSE;
}
...
int height;
{
 ...
 if (fread(&input_codesize, 1, 1, file) != 1)
  return 0;
  
 if (input_codesize == 0 || input_codesize >= MAX_LZW_BITS)
 {
...
bitfield_t *bitfield;
{
 ...
 if (fread(&block_len, 1, 1, bitfield->file) != 1 || (block_len == 0) ||
     fread(bitfield->data + 2, (int)block_len, 1, bitfield->file) != 1)
 {	 
  bitfield->error = TRUE;
  Xc_WARNING(("Unexpected end of LZW data"));
  return FALSE;
