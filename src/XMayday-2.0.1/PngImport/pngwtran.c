...
void
png_do_write_transformations(png_ptr)  png_structp png_ptr;
{
...
void
png_do_pack(row_info, row, bit_depth)  png_row_infop row_info; png_bytep row; png_byte bit_depth;
{
  ...
  case 1:
   {
    ...
    mask = 0x80;
    v = 0;
    for (i = 0; i < (png_int_32)row_info->width; i++)
    {
     if (*sp)
      v |= mask;
     sp++;
...
/* shift pixel values to take advantage of whole range.  Pass the
   ...
   data to 0 to 15. */
void
png_do_shift(row_info, row, bit_depth)  png_row_infop row_info; png_bytep row; png_color_8p bit_depth;
{
 if (row && row_info &&
...
/* remove filler byte */
void
png_do_write_filler(row_info, row, flags)  png_row_infop row_info; png_bytep row;
png_byte flags;
{
