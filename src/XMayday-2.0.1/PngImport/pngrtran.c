...
void
png_set_background(png_ptr, background_color, background_gamma_code, need_expand, background_gamma)  png_structp png_ptr;
...
void
png_set_strip_16(png_ptr)  png_structp png_ptr;
{
...
void
png_set_dither(png_ptr, palette, num_palette, maximum_colors, histogram, full_dither)  png_structp png_ptr; png_colorp palette;
...
void
png_set_gamma(png_ptr, screen_gamma, file_gamma)  png_structp png_ptr; double screen_gamma;
double file_gamma;
{
...
/* expand paletted images to rgb, expand grayscale images of
   ...
   to alpha channels */
void
png_set_expand(png_ptr)  png_structp png_ptr;
{
...
void
png_set_gray_to_rgb(png_ptr)  png_structp png_ptr;
{
...
/* initialize everything needed for the read.  This includes modifying
   the palette */
void
png_init_read_transformations(png_ptr)  png_structp png_ptr;
{
 int color_type;
...
void
png_read_transform_info(png_ptr, info_ptr)  png_structp png_ptr; png_infop info_ptr;
{
#if defined(PNG_READ_EXPAND_SUPPORTED)
...
void
png_do_read_transformations(png_ptr)  png_structp png_ptr;
{
#if defined(PNG_READ_EXPAND_SUPPORTED)
 if ((png_ptr->transformations & PNG_EXPAND) &&
...
void
png_do_unpack(row_info, row)  png_row_infop row_info; png_bytep row;
{
 int shift;
 png_bytep sp, dp;
 png_uint_32 i;
...
void
png_do_unshift(row_info, row, sig_bits)  png_row_infop row_info; png_bytep row;
png_color_8p sig_bits;
{
     ...
     row_info->color_type != PNG_COLOR_TYPE_PALETTE)
 {
  ...
  for (i = 0; i < (png_uint_32)channels; i++)
  {
...
void
png_do_chop(row_info, row)  png_row_infop row_info; png_bytep row;
{
 png_bytep sp, dp;
 png_uint_32 i;
...
/* add filler byte */
void
png_do_read_filler(row_info, row, filler, flags)  png_row_infop row_info; png_bytep row;
...
void
png_do_gray_to_rgb(row_info, row)  png_row_infop row_info; png_bytep row;
{
 png_bytep sp, dp;
 png_uint_32 i;
...
/* build a grayscale palette.  Palette is assumed to be 1 << bit_depth
   ...
   of code. */
void
png_build_grayscale_palette(bit_depth, palette)  int bit_depth; png_colorp palette;
{
 int num_palette;
 int color_inc;
 int i;
...
void
png_correct_palette(png_ptr, palette, num_palette)  png_structp png_ptr; png_colorp palette;
int num_palette;
{
...
/* replace any alpha or transparency with the supplied background color.
   ...
   elsewhere */
void
png_do_background(row_info, row, trans_values, background, background_1, gamma_table, gamma_from_1, gamma_to_1, gamma_16, gamma_16_from_1, gamma_16_to_1, gamma_shift)  png_row_infop row_info; png_bytep row;
...
void
png_do_gamma(row_info, row, gamma_table, gamma_16_table, gamma_shift)  png_row_infop row_info; png_bytep row;
...
void
png_do_expand_palette(row_info, row, palette, trans, num_trans)
...
void
png_do_expand(row_info, row, trans_value)  png_row_infop row_info; png_bytep row;
...
#if defined(PNG_READ_DITHER_SUPPORTED)
void
png_do_dither(row_info, row, palette_lookup, dither_lookup)  png_row_infop row_info; png_bytep row;
...
void
png_build_gamma_table(png_ptr)  png_structp png_ptr;
{
