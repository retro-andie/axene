...
png_structp
png_create_read_struct(user_png_ver, error_ptr, warn_fn, error_fn)  png_const_charp user_png_ver; voidp error_ptr;
...
void
png_read_init(png_ptr)  png_structp png_ptr;
{
...
void
png_read_info(png_ptr, info)  png_structp png_ptr; png_infop info;
{
...
void
png_read_update_info(png_ptr, info)  png_structp png_ptr; png_infop info;
{
...
void
png_start_read_image(png_ptr)  png_structp png_ptr;
{
   if (!(png_ptr->flags & PNG_FLAG_ROW_INIT))
      png_read_start_row(png_ptr);
}
...
void
png_read_row(png_ptr, row, dsp_row)  png_structp png_ptr; png_bytep row; png_bytep dsp_row;
{
   int ret;
   if (!(png_ptr->flags & PNG_FLAG_ROW_INIT))
      png_read_start_row(png_ptr);
...
void
png_read_rows(png_ptr, row, display_row, num_rows)  png_structp png_ptr; png_bytepp row;
...
void
png_read_image(png_ptr, image)  png_structp png_ptr; png_bytepp image;
{
   png_uint_32 i;
   int pass, j;
   png_bytepp rp;
...
void
png_read_end(png_ptr, info)  png_structp png_ptr; png_infop info;
{
   png_byte chunk_start[8];
   png_uint_32 length;
   png_uint_32 crc;
...
void
png_destroy_read_struct(png_ptr_ptr, info_ptr_ptr, end_info_ptr)  png_structpp png_ptr_ptr; png_infopp info_ptr_ptr;
...
void
png_read_destroy(png_ptr, info, end_info)  png_structp png_ptr; png_infop info; png_infop end_info;
{
   int i;
   jmp_buf tmp_jmp;
