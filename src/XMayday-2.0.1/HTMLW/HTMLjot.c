...
int width, height;
{
  ...
  if (jptr->pix != (Pixmap)NULL)
  {
    ...
    XFillRectangle(XtDisplay(w), jptr->pix, hw->html.drawGC, 0, 0,
...
XEvent *event;
{
  ...
  XCopyArea(XtDisplay(w), jptr->pix, XtWindow(w), hw->html.drawGC,
	    ExEvent->x, ExEvent->y, ExEvent->width, ExEvent->height,
	    ExEvent->x, ExEvent->y);
}
...
Widget w;
{
  ...
  Xc_TRACE(("Packaging up %d points\n", jptr->stroke_cnt));
