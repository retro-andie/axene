...
int type;
{
 ...
 case M_ANCHOR:
  ...
  break;
 case M_PARAGRAPH:
  Xc_TRACE(("M_PARAGRAPH"));
  break;
 case M_ADDRESS:
  Xc_TRACE(("M_ADDRESS"));
  break;
 case M_PLAIN_TEXT:
  Xc_TRACE(("M_PLAIN_TEXT"));
  break;
 case M_LISTING_TEXT:
  Xc_TRACE(("M_LISTING_TEXT"));
  break;
 case M_UNUM_LIST:
  Xc_TRACE(("M_UNUM_LIST"));
  break;
 case M_NUM_LIST:
  Xc_TRACE(("M_NUM_LIST"));
  break;
 case M_MENU:
  ...
  break;
 case M_DIRECTORY:
  Xc_TRACE(("M_DIRECTORY"));
  break;
 case M_LIST_ITEM:
  Xc_TRACE(("M_LIST_ITEM"));
  break;
 case M_DESC_LIST:
  Xc_TRACE(("M_DESC_LIST"));
  break;
 case M_DESC_TITLE:
  Xc_TRACE(("M_DESC_TITLE"));
  break;
 case M_DESC_TEXT:
  Xc_TRACE(("M_DESC_TEXT"));
  break;
 case M_IMAGE:
  Xc_TRACE(("M_IMAGE"));
  break;
 ...
 case M_BASE:
  ...
  break;
 case M_LINEBREAK:
  Xc_TRACE(("M_LINEBREAK"));
  break;
 case M_BLOCKQUOTE:
  Xc_TRACE(("M_BLOCKQUOTE"));
  break;
 default:
  Xc_TRACE(("UNKNOWN %d", type));
  break;
 }
#endif
}
...
struct ele_rec *list;
{
 ...
 while (current != NULL)
 {
  ...
  {
  case E_IMAGE:
#if 0				/* done in HelpWidget.c */
   if (eptr->pic_data)
   {
    if (eptr->pic_data->text)
     free(eptr->pic_data->text);
    if (eptr->pic_data->src)
  ...
  case E_WIDGET:
   if (eptr->widget_data)
    free(eptr->widget_data);
   break;
  case E_TABLE:
   if (eptr->pic_data)
   {
    if (eptr->table_data->numColumns && eptr->table_data->numRows)
     free(eptr->table_data->table);
    if (eptr->table_data->caption)
     free(eptr->table_data->caption);
    free(eptr->table_data);
   }
   break;
  case E_HRULE:
  case E_ANCHOR:
 ...
 */
 ...
 if (ll == NULL)
 {
  Xc_FATAL(("cannot allocate space for line list"));
 }
...
char *href;
{
 ...
 if (hptr == NULL)
 {
  ...
  if (hptr == NULL)
  {
   Xc_TRACE(("cannot extend internal href list"));
   return *list;
  }
...
char *src;
{
 ...
 if (iptr == NULL)
 {
  ...
  if (iptr == NULL)
  {
   Xc_WARNING(("cannot extend visited delayed images list"));
   return list;
  }
