...
#define HTML_NO_CURSOR

#include "xcalibur.h"
#include "HTMLP.h"
#include <Xm/DrawingA.h>
#include <Xm/ScrollBar.h>
#ifndef HTML_NO_CURSOR
...
static XtActionsRec actionsList[] =
{
  ...
  { "key-event",       (XtActionProc) ManageKeyEvent },
...
static XtResource resources[] =
{
  {	
    ...
    XtRString, "-*-helvetica-bold-r-normal-*-8-*-*-*-*-*-iso8859-1"
  }, {	
    WbNaddressFont,
    ...
    XtRString, "-*-courier-medium-r-normal-*-8-*-*-*-*-*-iso8859-1"
  }, /* end amb */ {
    WbNbigFont,
    WbCBigFont, XtRFontStruct, sizeof (XFontStruct *),
    XtOffset (HTMLWidget, html.big_font),
    XtRString, "-*-helvetica-medium-r-normal-*-16-*-*-*-*-*-iso8859-1"
  }, {
    WbNsmallFont,
    WbCSmallFont, XtRFontStruct, sizeof (XFontStruct *),
    XtOffset (HTMLWidget, html.small_font),
    XtRString, "-*-helvetica-medium-o-normal-*-10-*-*-*-*-*-iso8859-1"
  }, {
    ...
    WbNextraData, WbCExtraData, XtRPointer, 
    sizeof (XtPointer),
    XtOffset (HTMLWidget, html.extra_data),
    XtRImmediate, (caddr_t) NULL
  }, 
};
...
char *bgname;
{
  ...
  if (!bgname || !*bgname || hw->html.delay_images)
    return;
  ...
  if (hw->html.resolveImage != NULL)
  {
    ...
    if (pic_data != NULL && pic_data->image_data!=NULL)
    {
	...
	pic_data->clip = XCreatePixmapFromBitmapData
	  (XtDisplay(hw), XtWindow(hw->html.view), pic_data->clip_data,
	   pic_data->width, pic_data->height, 1, 0, 1);
      }
      else
...
void hw_do_color(w, att, cname)
Widget w;
char *att;
char *cname;
{
  color_cells_t   cell, cell2;
  HTMLWidget hw = (HTMLWidget) w;
  ...
  if (!F(GlobColormap).allocate_named_color(GlobColormap, cname, &cell))
   return;
      
  if (!strcasecmp(att,"text"))
  {
    hw->manager.foreground = cell.pixel;
  }
  if (!strcasecmp(att,"bgcolor"))
  {
   XColor col, fg, sel, ts, bs;
   XmColorProc calc;

   col.pixel = cell.pixel;
   col.red = ((unsigned short) cell.red);
   col.green = ((unsigned short) cell.green);
   col.blue = ((unsigned short) cell.blue);
   ...
   F(GlobColormap).get_closest_color(GlobColormap, &cell2, 
				     ts.red, ts.green, ts.blue);
   hw->manager.top_shadow_color = cell2.pixel;
   F(GlobColormap).get_closest_color(GlobColormap, &cell2, 
				     bs.red, bs.green, bs.blue);
   hw->manager.bottom_shadow_color = cell2.pixel;
   
   hw->core.background_pixel = cell.pixel;
   hw->html.view->core.background_pixel = cell.pixel ;
   hw->html.activeAnchor_bg = cell.pixel;
  }

  if (!strcasecmp(att,"link"))
  {
    hw->html.anchor_fg = cell.pixel;
  }

  if (!strcasecmp(att,"vlink"))
  {
    hw->html.visitedAnchor_fg = cell.pixel;
  }

  if (!strcasecmp(att,"alink"))
  {
    hw->html.activeAnchor_fg = cell.pixel;
  }
...
XEvent *event;
{
  ...
  while (((XCheckWindowEvent(XtDisplay(w), 
			     XtWindow(w), ExposureMask, &NewEvent) == True)))
  {
    ExEvent = (XExposeEvent *)&NewEvent;
    ...
    if (x1 < (ExEvent->x + ExEvent->width))
    {
      x1 = ExEvent->x + ExEvent->width;
    }
    width = x1 - x;
    
    if (y1 < (ExEvent->y + ExEvent->height))
    {
      y1 = ExEvent->y + ExEvent->height;
    }
    height = y1 - y;
  }
  ...
  if (hw->html.drawGC == NULL)
  {
    ...
    hw->html.drawGC = XCreateGC(XtDisplay(hw), XtWindow(hw->html.view),
				valuemask, &values);
  }
...
int value;
{
  ...
  if (w == hw->html.vbar)
  {
    ...
    if (value > hw->html.scroll_y)
    {
      ...
      {
	hw->html.scroll_y = value;
	XClearArea(XtDisplay(hw), XtWindow(hw->html.view), 0, 0,
		   hw->html.view_width, hw->html.view_height, False);
	ViewRedisplay(hw, 0, 0, hw->html.view_width, hw->html.view_height);
      }
      else
      {
	XCopyArea(XtDisplay(hw), XtWindow(hw->html.view),
		  XtWindow(hw->html.view), hw->html.drawGC, 0, dy,
		  hw->html.view_width, hw->html.view_height - dy, 0, 0);
	hw->html.scroll_y = value;
	XClearArea(XtDisplay(hw), XtWindow(hw->html.view),
		   0, (int)hw->html.view_height - dy, hw->html.view_width,
		   dy, False);
	ViewRedisplay(hw, 0, (int)hw->html.view_height - dy,
	...
	else
	{
	  XCopyArea(XtDisplay(hw), XtWindow(hw->html.view),
		    ...
		    hw->html.view_width, hw->html.view_height - dy, 0, dy);
	  hw->html.scroll_y = value;
	  XClearArea(XtDisplay(hw), XtWindow(hw->html.view), 0,0,
  ...
  else 
  {
	...
	{
	  XCopyArea(XtDisplay(hw), XtWindow(hw->html.view),
		    XtWindow(hw->html.view), hw->html.drawGC, dx, 0,
		    hw->html.view_width - dx, hw->html.view_height, 0, 0);
	  hw->html.scroll_x = value;
	  XClearArea(XtDisplay(hw), XtWindow(hw->html.view),
		     (int)hw->html.view_width - dx, 0,
		     dx, hw->html.view_height, False);
	  ViewRedisplay(hw, (int)hw->html.view_width - dx, 0,
	...
	if (value < hw->html.scroll_x)
	{
	  ...
	  else
	  {
	    XCopyArea(XtDisplay(hw), XtWindow(hw->html.view),
		      XtWindow(hw->html.view), hw->html.drawGC, 0, 0,
		      hw->html.view_width - dx, hw->html.view_height, dx, 0);
	    hw->html.scroll_x = value;
	    XClearArea(XtDisplay(hw), XtWindow(hw->html.view), 0, 0,
		       dx, hw->html.view_height, False);
   ...
   * Flush any Copyed or Cleared text first. XFlush(XtDisplay(hw));
   */
  ScrollWidgets(hw);
  
  /*
   * Remap the widgets to their new location MapWidgets(hw);
   */
}

/*
 * HTMLGetScrollPos - Add by Xibios
 */

void HTMLGetScrollPos(hw, vx, vy)
Widget hw;
int *vx;
int *vy;
{
  if (vx)
    *vx = ((HTMLWidget)hw)->html.scroll_x;
  if (vy)
    *vy = ((HTMLWidget)hw)->html.scroll_y;
}

/*
 * HTMLSetScrollPos - Add by Xibios
 */ 

void HTMLSetScrollPos(hw, vx, vy)
Widget hw;
int vx;
int vy;
{
  ...
  if (vx != ((HTMLWidget)hw)->html.scroll_x)
  {
    XtSetArg(args[0], XmNvalue, vx);
    XtSetValues(((HTMLWidget)hw)->html.hbar, args, 1);
    ScrollToPos(((HTMLWidget)hw)->html.hbar, (HTMLWidget)hw, vx);
  }
  
  if (vy != ((HTMLWidget)hw)->html.scroll_y)
  {
    XtSetArg(args[0], XmNvalue, vy);
    XtSetValues(((HTMLWidget)hw)->html.vbar, args, 1);
    ScrollToPos(((HTMLWidget)hw)->html.vbar, (HTMLWidget)hw, vy);
  }
}
 ...
 * Size them later.
 */
static void CreateScrollbars (hw)
HTMLWidget hw;
{
  Arg arg[20];
  Cardinal argcnt;
  ...
  if (hw->html.view == NULL)
  {
    ...
    hw->html.view = 
      XtCreateWidget("View", xmDrawingAreaWidgetClass, (Widget)hw, arg, argcnt);
 ...
 */
static Dimension HbarHeight (hw)
HTMLWidget hw;
{
  Arg arg[4];
  Cardinal argcnt;
 ...
 */
static void ConfigScrollBars (hw)
HTMLWidget hw ;
{
  Arg arg[20];
  Cardinal argcnt;
  ...
  if (hw->html.use_vbar == True)
  {
    ...
    {
      Xc_TRACE(("view_height %d, doc_height %d", 
    ...
    Xc_TRACE(("computed ss to be %d", ss));
    ...
    Xc_TRACE(("real slider size %d", ss));
#endif
  }
  ...
  if (hw->html.use_hbar == True)
  {
     ...
     * the viewing area
     */
    XtResizeWidget(hw->html.hbar,  hw->html.view_width + 
		   (2 * hw->manager.shadow_thickness),
...
HTMLWidget hw;
{
  ...
  Xc_TRACE(("calling in ReformatWindow"));
...
HTMLWidget new;
{
  ...
  new->html.background_SAVE = new->core.background_pixel = 
    new->html.view->core.background_pixel;
  new->html.bgmap_SAVE = None;
  new->html.bgclip_SAVE = None;
  new->html.bg_image=0;

  new->html.forms_tabbing = False;
...
{
  Xc_TRACE(("Redrawing (%d,%d) %dx%d", x, y, width, height));
}
 ...
 */
static void ViewRedisplay(hw, x, y, width, height)
HTMLWidget hw;
int x, y;
int width, height;
{
 ...
 */
 if (hw->html.drawGC != NULL)
 {
  ...
  rect[0].width = width; rect[0].height = height;
  
  XSetClipRectangles(XtDisplay(hw), hw->html.drawGC, 
		     0, 0, rect, 1, Unsorted);
  
  hw->html.redraw_x = x; hw->html.redraw_y = y;
  hw->html.redraw_width = width; hw->html.redraw_height = height;
...
void HTMLViewClearAndRefresh(hw)
Widget hw;
{
  ViewClearAndRefresh((HTMLWidget)hw);
}

char *HTMLGetTitle(hw)
Widget hw;
{
  return ((HTMLWidget)hw)->html.title;
}
...
HTMLWidget hw;
{
...
#ifndef NTRACE
  {
    int ss;
    XtVaGetValues(hw->html.vbar, XmNsliderSize, &ss, NULL);
    Xc_TRACE(("leaving; slider size %d", ss));
  }
#endif
...
Cardinal *num_params;
{
      ...
      (hw->html.new_end != NULL))
  {
    ...
    if (atoms == NULL)
    {
      Xc_TRACE(("cannot allocate atom list"));
      return;
    }
...
{
  HTMLWidget hw = (HTMLWidget)XtParent(w);
  WbpointerMotionCallbackData cbdata;
  struct ele_rec *eptr;
  int epos, x, y;
  ...
  if (event->type == MotionNotify)
  {
    ...
    /* put keyboard focus back into hot keys */
    if(hw->html.forms_tabbing)
    {
      XtSetKeyboardFocus((Widget)hw, hw->html.view);
      hw->html.forms_tabbing = False;
    }
  }
  else
  {
  ...
  {
    if ((eptr == NULL || (eptr != NULL && eptr->anchorHRef == NULL)) &&
	hw->html.cached_tracked_ele != NULL)
    {
      hw->html.cached_tracked_ele = NULL;
      cbdata.event = event;
...
static void ManageKeyEvent(w, event, params, num_params)
Widget w;
XEvent *event;
String *params;         /* unused */
Cardinal *num_params;   /* unused */
{
  ...
  if (event->type == KeyPress || event->type == KeyRelease)
  {
    ...
    case XK_End:
    case XK_Begin:
      event->xkey.window = XtWindow(hw->html.vbar);
      XtDispatchEvent(event);
/*      XtDispatchEventToWidget(hw->html.vbar, event); */
      break;
    case XK_Right:
    case XK_Left:
      event->xkey.window = XtWindow(hw->html.hbar);
      XtDispatchEvent(event);
/*      XtDispatchEventToWidget(hw->html.hbar, event); */
      break;      
    }
  }
  ...
  if (event->type == ButtonPress || event->type == ButtonRelease)
  {
   ...
   {
    event->xbutton.window = XtWindow(hw->html.vbar);
    XtDispatchEvent(event);
   }
  }  
}
...
void GetMailtoKludgeInfo(url,subject)
char **url;
char **subject;
{
  *url = mailToKludgeURL;
 ...
 * is pressed
 */
static void _HTMLInput( w, event, params, num_params)
Widget w ;
XEvent *event ;
String *params;		/* unused */
Cardinal *num_params;	/* unused */
{   
  ...
  if (event->type == ButtonRelease)
  {
	...
	/* HTMLSetAppInsensitive((Widget) hw); */ /* remove by Xibios2 */
	    ...
	    (hw->html.resolveDelayedImage != NULL))
	{
	  ...
	  else
	  {
	     ...
	     */
	    if ((eptr->edata != NULL) &&
		(strncmp(eptr->edata, INTERNAL_IMAGE, strlen(INTERNAL_IMAGE))
		 == 0))
	    {
	...
	else if ((eptr->pic_data != NULL) && (eptr->pic_data->delayed) &&
		 (eptr->anchorHRef != NULL) &&
		 (IsIsMapForm(hw, eptr->anchorHRef)))
	{
	  ...
	  {
	    if ((eptr->pic_data != NULL) && (eptr->pic_data->ismap) &&
		(eptr->anchorHRef != NULL) &&
		(IsIsMapForm(hw, eptr->anchorHRef)))
	    {
	      int form_x, form_y;
		...
		alloced = 1;
		sprintf(buf, "%s?%d,%d", eptr->anchorHRef,
...
HTMLWidget new ;
{
      ...
      (request->html.footer_text != current->html.footer_text))
  {
     ...
     */
    {
      new->html.scroll_x = 0;
      new->html.scroll_y = 0;
      new->html.max_pre_width = 
	DocumentWidth(new, new->html.html_objects);
      ReformatWindow(new);
 ...
 */
static void CallLinkCallbacks(hw)
HTMLWidget hw;
{
  struct mark_up *mptr;
  LinkInfo l_info;
 ...
 * the passed HREF.
 */
static void RecolorInternalHRefs(hw, href)
HTMLWidget hw;
char *href;
{
  struct ele_rec *start;
...
int *format;
{
  ...
  if (*target == XA_TARGETS(d))
  {
    ...
    XmuConvertStandardSelection( w, hw->html.selection_time, selection, 
 ...
 */
char *HTMLGetText (w, pretty, url, time_str)
Widget w;
int pretty;
char *url;
char *time_str;
 ...
 */
int HTMLIdToPosition(w, element_id, x, y)
Widget w;
int element_id;
int *x;
int *y;
 ...
 * If there is no such element, x=0, y=0 and -1 is returned.
 */
int HTMLAnchorToPosition(w, name, x, y)
Widget w;
char *name;
int *x;
int *y;
...
/* 
 * Exportable function to add a HRef to visited href list
 * Add by Xibios2 from Axene
 */
void HTMLAddHRef(w, anchor)
Widget w;
char *anchor;
{
  HTMLWidget hw = (HTMLWidget)w;
  ...
  if (anchor)
    AddHRef(&(hw->html.my_visited_hrefs), anchor);
}
 ...
 */
char **HTMLGetImageSrcs(w, num_srcs)
Widget w;
int *num_srcs;
{
  HTMLWidget hw = (HTMLWidget)w;
 ...
 */
LinkInfo *HTMLGetLinks(w, num_links)
Widget w;
int *num_links;
{
  HTMLWidget hw = (HTMLWidget)w;
...
void HTMLFreeImageInfo(w)
Widget w;
{
  HTMLWidget hw = (HTMLWidget)w;
  
  FreeColors(XtDisplay(w), (installed_colormap ? installed_cmap :
			    GlobColormap->cmap));
  FreeImages(hw);
}

		       
void HTMLFreeWidgetInfo(ptr)
void *ptr;
{
 ...
 * function.
 */
void HTMLRetestAnchors(w, testFunc)
Widget w;
 ...
 */
void HTMLSetSelection (w, start, end)
Widget w;
ElementRef *start;
ElementRef *end;
{
 ...
 * to the user to free.
 */
char * HTMLGetTextAndSelection (w, startp, endp, insertp)
Widget w;
char **startp;
char **endp;
char **insertp;
{
  ...
  *insertp = NULL;
  ...
  if (text == NULL)
  {
    Xc_TRACE(("No space for return string"));
 ...
 * to that anchor.
 */
void HTMLSetText(w, text, header_text, footer_text, element_id, 
		 target_anchor, ptr)
Widget w;
char *text;
char *header_text;
...
void *ptr;
{
  ...
  if (text == HTML_NO_RELOAD) goto anchor_jump;
  ...
  if (text != NULL)
  {
    ...
    FreeColors(XtDisplay(hw),
	       (installed_colormap ?
		installed_cmap : GlobColormap->cmap));
    FreeImages(hw);
  ...
  if (header_text != NULL)
  {
    ...
    hw->html.html_header_objects =
      HTMLParse(hw->html.html_header_objects, hw->html.header_text,hw);
  }
  if (footer_text != NULL)
  {
    if (*footer_text == '\0')
    ...
    hw->html.html_footer_objects =
      HTMLParse(hw->html.html_footer_objects, hw->html.footer_text,hw);
  }
 ...
 anchor_jump:
  ...
  Xc_TRACE(("calling in HTMLSetText"));
 ...
 */
int HTMLSearchText (w, pattern, m_start, m_end, backward, caseless)
Widget w;
char *pattern;
ElementRef *m_start;
ElementRef *m_end;
 ...
 */
void HTMLDrawBackgroundImage(wid, x, y, width, height)
Widget wid;
int x;
int y;
int width;
int height;
{  
  int w_whole = 0, h_whole = 0, start_width = 0, start_height = 0;
  int w_start_offset = 0, h_start_offset = 0, w_whole_tiles = 0;
  int h_whole_tiles = 0, end_width = 0, end_height = 0, w, h;
  int destx = 0, desty = 0;
  HTMLWidget hw = (HTMLWidget) wid;
  
  if(!hw || (x<0) || (y<0) || width<=0 || height<=0 ||
     !hw->html.bg_width || !hw->html.bg_height || !hw->html.bgmap_SAVE ||
     (hw->html.view_width < width) || (hw->html.view_height < height) ||
     !hw->html.bg_image)
    return;
  ...
  {
    XSetClipMask(XtDisplay(hw), hw->html.drawGC, hw->html.bgclip_SAVE);
  }
  for (h=0; h<h_whole; h++)
  {
    destx=x;
	...
	XSetClipOrigin(XtDisplay(hw), hw->html.drawGC,
      ...
      XCopyArea(XtDisplay(wid),	hw->html.bgmap_SAVE,
		...
		hw->html.drawGC, w_start_offset*(!w),
		h_start_offset*(!h), 
		(!w ? (start_width ? start_width :
		       hw->html.bg_width) : 
		 ((w+1)==w_whole ?
		  (end_width ? end_width : hw->html.bg_width) : 
		  hw->html.bg_width)),
		(!h ?
		 (start_height ? start_height : hw->html.bg_height) :
		 ((h+1)==h_whole ?
		  (end_height ? end_height : hw->html.bg_height) :
		  hw->html.bg_height)),
		destx, desty);
      destx+=(!w ? (start_width ? start_width : hw->html.bg_width) :
	      hw->html.bg_width);
    }
    desty+=(!h ? (start_height ? start_height : hw->html.bg_height) :
	    hw->html.bg_height);
  }
  if (hw->html.bgclip_SAVE!=None)
  {
