...
{
 11  * _INCH,			/* page_height */
 8.5 * _INCH,			/* page_width  */
 0.9 * _INCH,			/* top_margin  */
 0.7 * _INCH,
 0.9 * _INCH,
 0.9 * _INCH
};
...
#else  /* for XMayday */
static float fontsizes[19];
static int fontascent[19];
#endif
...
int HTML_Draft_Mode	   = 0;	/* Flag whether draft mode enabled */
int HTML_Color_Mode	   = 1; /* Flag whether color mode enabled */

extern int installed_colormap;
extern Colormap installed_cmap;
extern int FormatAll();
static void PSprintEleRec();
...
va_dcl
{
 ...
 if (PS_size - PS_len < 1024) 
 {
  ...
  {
   Xc_WARNING(("PSprintf malloc failed"));
   return EOF;
  }
  PS_string = s;
 }
...
int flush;
{
 ...
 {
  hexline[PS_hexi++] = 
   (char) digit[((unsigned) val >> (unsigned) 4) & (unsigned) 0x0f];
  hexline[PS_hexi++] = 
   (char) digit[(unsigned) val & (unsigned) 0x0f];
 }
...
#ifdef Xc_XMAYDAY
/* 
* PScalculFont compute fontsizes and fontascent with real XFontStruct params
* Mofication by Xibios2 from Axene 
*/
static void PScalculFont(hw, fontf)
HTMLWidget hw;
int fontf;		/* font family */
{
 ...
 for(i = 0; i < 19; i++)
 {
  ...
  fontsizes[i] = (float)font->max_bounds.ascent;
  fontascent[i] = font->ascent;
  switch(fontf)
  {
  case 1:			/* Helvetica */
   fontsizes[i] = (fontsizes[i] * 0.92);
   break;
  case 2:			/* NewCentury */
   fontsizes[i] = (fontsizes[i] * 0.88);
   break;
  case 0:			/* Times */
  case 3:			/* Lucida */
  default:
  ...
  /*    fontsizes[i] = (int)((double)(font->ascent+font->descent)
  * Points_Pixel);
  fontascent[i] = (int)((double)font->ascent * Points_Pixel); */
 }
}
#endif
...
*
*/
static void PSfont(hw, font, fontfamily)
HTMLWidget hw;
XFontStruct *font;
int fontfamily;
{
 ...
 static PS_fontstyle fontstyle[19] = 
 {
  RF, IF, BF, FF, BF, BF, BF, BF, BF,
  BF, IF, FF, FF, FB, FI, FB, FI, RF, IF
 };
 ...
 * defined in HTML.c)
 */
 static int fontsizes[4][3][19] = 
 {
  /* times font sizes */
  {
   {14, 14, 14, 14, 18, 17, 14, 12, 10, 8, 14, 12, 12, 14, 14, 12, 12, 16, 12},
   {17, 17, 17, 17, 24, 18, 17, 14, 12, 10, 17, 14, 12, 17, 17, 14, 14, 19, 15},
   {20, 20, 20, 20, 25, 24, 20, 18, 17, 14, 20, 18, 12, 20, 20, 18, 18, 22, 18}
  },
  /* helvetica sizes */
  {
   {14, 14, 14, 14, 18, 17, 14, 12, 10, 8, 14, 12, 12, 14, 14, 12, 12, 16, 12},
  ...
  {
   {14, 14, 14, 14, 18, 17, 14, 12, 10, 8, 14, 12, 12, 14, 14, 12, 12, 16, 12},
   {18, 18, 18, 18, 24, 18, 17, 14, 12, 10, 18, 14, 12, 18, 18, 14, 14, 19, 15},
   {20, 20, 20, 20, 25, 24, 20, 18, 17, 14, 20, 18, 12, 20, 20, 18, 18, 22, 18}
  },
  /* lucida sizes */
  {
   {14, 14, 14, 14, 18, 17, 14, 12, 11, 10, 14, 12, 12, 14, 14, 12, 12, 16, 12},
   {17, 17, 17, 17, 24, 18, 17, 14, 12, 10, 17, 14, 12, 17, 17, 14, 14, 19, 15},
   {20, 20, 20, 20, 25, 24, 20, 18, 17, 14, 20, 18, 12, 20, 20, 18, 18, 22, 18}
  }
 };
 ...
 */
 static int fontascent[4][3][19] =
 {
  /*rg, itl, bld, fix,  h1,  h2,  h3,  h4,  h5,  h6,
    add, pla, lis, fixbold, fixital, plabold, plaital, */
  /* times */
  {
   {12, 11, 12, 10, 15, 14, 12, 10, 8, 6, 11, 9, 10, 10, 10, 9, 9, 14, 10},
   {13, 13, 14, 12, 20, 15, 14, 12, 10, 8, 13, 10, 10, 12, 12, 10, 10, 15, 11},
   {16, 15, 15, 13, 21, 20, 15, 15, 14, 12, 15, 13, 10, 13, 13, 13, 13, 18, 14}
  },
  /* helvetica */
  {
   {12, 12, 12, 10, 15, 14, 12, 10, 9, 7, 12, 9, 10, 10, 10, 9, 9, 14, 10},
   {14, 14, 14, 12, 22, 15, 14, 12, 10, 9, 14, 10, 10, 12, 12, 10, 10, 16, 11},
   {16, 16, 16, 13, 22, 22, 16, 15, 14, 12, 16, 13, 10, 13, 13, 13, 13, 18, 14}
  },
  /* new century schoolbook */
  {
   {12, 12, 13, 10, 16, 14, 13, 10, 9, 7, 12, 9, 10, 10, 10, 9, 9, 14, 10},
   {16, 14, 16, 13, 22, 16, 14, 13, 10, 9, 14, 10, 10, 13, 13, 10, 10, 18, 14},
   {17, 16, 17, 13, 22, 22, 17, 16, 14, 13, 16, 13, 10, 13, 13, 13, 13, 19, 15}
  },
  /* lucida bright */
  {
   {11, 11, 11, 11, 15, 14, 11, 10, 9, 7, 11, 9, 10, 11, 10, 9, 9, 13, 8},
   {14, 15, 14, 13, 20, 15, 14, 11, 10, 7, 15, 11, 10, 13, 13, 11, 10, 16, 12},
   {17, 17, 17, 16, 21, 20, 17, 15, 14, 11, 17, 14, 10, 16, 13, 14, 13, 19, 15}
  }
 };
#endif
 ...
 if (font == hw->html.font)
  ...
  style = 16;
 else if (font == hw->html.big_font)
  style = 17;
 else if (font == hw->html.small_font)
  style = 18;
 ...
 fn = fontstyle[style];
 fs = fontsizes[style];
 PS_fontascent = fontascent[style];
#else
 /* check size, by looking at the size of the regular font */
 size = 1;
 ...
 if (n_saved_ftns > 0) 
 {
  ...
  for (i = 0; n_saved_ftns; n_saved_ftns--, i++) 
  {
   PSprintf("newpath %.2f %.2f M RF %.2f SF (%d) S 3 -2 R RF %d SF (%s) S\n", 
	    page_dimens.left_margin,
	    page_dimens.bot_margin + 5 + 
	    (n_saved_ftns - 1) * footnote_ptsize,
...
/*
* PSinit_image - handle image encoding
*
* Addition by Xibios from Axene.
*
*/
static void PSinit_image()
{
 static char *txt[] =
 {
  "\n% Image procs",
  "/dd{XMaydayDict 3 1 roll put} D",
  "/xdd{exch dd} D",
  "/dg{XMaydayDict exch get} D",
  "/colorimage where{pop}{% colorimage emulation (8bits color samples only)",
  "/_glue{2{aload length dup 2 add -1 roll}repeat add array astore cvx} D",
  "/$c2g[{}{}{dup length 3 idiv 0 1 2 index 1 sub{dup 3 mul 3 index 1",
  "index get .3 mul 4 index 2 index 1 add get .59 mul add 4 index 2",
  "index 2 add get .11 mul add round cvi exch pop 3 index 3 1 roll put",
  "}for 0 exch getinterval}% RGB->Gray",
  "{dup length 4 idiv 0 1 2 index 1 sub{dup 4 mul 255 4 index 2 index",
  "get .3 mul 5 index 3 index 1 add get .59 mul add 5 index 3 index",
  "2 add get .11 mul add 5 index 3 index 3 add get add round cvi dup", 
  "255 gt{pop dup}if sub exch pop 3 index 3 1 roll put}for 0 exch",
  "getinterval}% CMYK->Gray",
  "] D",
  "/colorimage{exch pop $c2g exch 1 sub get exch _glue image} D}ifelse ",
  "/ASCIIHexDecode{currentfile $is readhexstring pop} D",
  "/DeviceCMYK 4 def",
  "/DeviceRGB 3 def",
  "/DeviceGray 1 def",
  ...
  "Height BitsPerComponent[Width 0 0 Height 0 0] Filter dg false ColorSpace",
  "dg end colorimage{currentfile read{62 eq{exit}if}{exit}ifelse}loop} D",
 };  
...
*/
static void PSheader(title, font, url, time_str)
char *title;
int font;
char *url;
char *time_str; 
{
 ...
 static char *txt[] =
 {
  ...
  "/HR {gsave 0.6 setlinewidth 0 RL stroke grestore } D",
 ...
 time_buf = ctime(&clock);
 time_buf[strlen(time_buf) - 1] = '\0';/* squiz \n at end */
 ...
 PSprintf("save\n");
 PSprintf("/XMaydayDict 100 dict def\nXMaydayDict begin\n\n");
 PSprintf("/D {def} def\n/E {exch} D\n");
 PSprintf("/RF {/%s} D\n", fontname[font*3]);
 PSprintf("/BF {/%s} D\n", fontname[font*3+1]);
 PSprintf("/IF {/%s} D\n", fontname[font*3+2]);
 ...
 if (HTML_Print_Headers) 
 {
  ...
  PSprintf("  newpath %.2f %.2f M %.2f 0 RL stroke\n", 
	   page_dimens.left_margin,
	   (page_dimens.bot_margin + page_dimens.text_height + MARGIN), 
	   page_dimens.text_width);
  PSprintf("  newpath %.2f %.2f M %.2f 0 RL stroke\n",
	   page_dimens.left_margin, page_dimens.bot_margin - MARGIN,
	   page_dimens.text_width);
  PSprintf("  BF 12 SF %.2f %.2f M (%s) S\n",
	   page_dimens.left_margin,
	   (page_dimens.bot_margin + page_dimens.text_height + MARGIN + 6),
	   title);
 ...
 PSinit_image();		/* addition by Xibios from Axene */
...
*
*/
static void PSmove_offset(offset)
int offset;
{
 if (offset != PS_offset) 
 {
...
*
* an element has a footnote if it is text or an image and its anchorHRef
* is not null. If the element is a textual element with an anchorHRef, 
* that has been split across lines then it should be followed by a linefeed
* element and a text element with the same anchorHRef.  In this case say
* that the element doesn't have a footnote so as to avoid duplicate footnotes.
 */
 static int has_footnote(el)
  struct ele_rec *el;
{
 int			rc = 0;
 struct ele_rec 	*next;
 ...
 * is not affected.
 * The reference is stored in a malloced array (which may need to be expanded)
 * to be output at the end of the page.
 */
 static void PSfootnote(href, height)
  char *href;
 double height;
{
 ...
 if (n_saved_ftns == ftn_array_size) 
 {
  ...
  if (footnotes == NULL) 
  {
   Xc_WARNING(("PSfootnote realloc failed"));
   return;
  }
 }
 ...
 *
 */
 static void PStext(hw, eptr, fontfamily, s)
  HTMLWidget hw;
 struct ele_rec *eptr;
 int fontfamily;
 String s;
{
 ...
 if (s2 == NULL) 
 {
  Xc_WARNING(("PStext malloc failed"));
  return;
 }
  
 /*  For each char in s, if it is a special char, insert double quote
...
*
*/
static void PSbullet(hw, eptr, fontfamily)
HTMLWidget hw;
struct ele_rec *eptr;
int fontfamily;
{
...
*
*/
static void PShrule(hw, eptr, fontfamily)
HTMLWidget hw;
struct ele_rec *eptr;
int fontfamily;
{
 int length = eptr->width;
 /*page_dimens.text_width / Points_Pixel;*/
...
*
*/
static int PStableCheckFootnotes(hw, eptr, ypos, yspace)
HTMLWidget hw;
struct ele_rec *eptr;
int ypos;
int yspace;
{
 TableInfo *table = eptr->table_data;
 TableField *field;
 struct ele_rec *rec;
 int x, y, maxheight;
 int footn = 0;
 ...
 maxheight = yspace;
 for(y = 0; y < table->numRows; y++)
 {
  for(x = 0; x < table->numColumns; x++)
  {
   field = &(table->table[y * table->numColumns + x]);
   ...
   do
   {
    ...
    if (rec->y > maxheight) break;

    if (rec->type == E_TABLE)
    {
     footn += PStableCheckFootnotes(hw, rec, ypos, maxheight);
     maxheight = yspace - 
      (footnote_space + (footnote_ptsize * footn) / Points_Pixel);
    }
    else
    {
     if (rec->y + rec->line_height > maxheight) break;
     if (has_footnote(rec))
     {
      footn++;
      maxheight = yspace - 
       (footnote_space + (footnote_ptsize * footn) / Points_Pixel);
      if (maxheight < (ypos + (yspace - ypos) / 2))
       return footn - 1;
     }
    }
...
static int PStableRange(hw, eptr, fontfamily, xpos, ypos, ytop, lmargin, 
...
int reserved_space;
{
 ...
 TableField *field;
 struct ele_rec *rec;
 int x, y, ystart, maxheight, bx, by;
 int np = 0;
 boolean recin;
 ...
 if (table->caption)
 {
  if (table->captionAlignment == ALIGN_TOP && 
      eptr->y >= ypos && eptr->y < *ytop) 
  {
   int tpos;
   
   tpos = (eptr->width - XTextWidth(table->captionFont, table->caption,
				    strlen(table->caption))) / 2;
   PSmoveto( xpos + tpos - lmargin, ypos + 
	    table->captionFont->ascent + 2 - PS_page_offset);
   PSfont(hw, table->captionFont, fontfamily);/* set font */
   PSprintf("(%s)S\n", table->caption);
   by = by + table->height - table->bheight;
  }
  if (table->captionAlignment == ALIGN_BOTTOM && 
      eptr->y + table->bheight >= ypos && eptr->y + table->bheight < *ytop) 
  {
   int tpos;
   
   tpos = (eptr->width - XTextWidth(table->captionFont, table->caption,
				    strlen(table->caption))) / 2;
   PSmoveto( xpos + tpos - lmargin, 
	    ypos + table->bheight + 
	    table->captionFont->ascent + 2 - PS_page_offset);
   PSfont(hw, table->captionFont, fontfamily);/* set font */
   PSprintf("(%s)S\n", table->caption);
 ...
 if (table->borders)
 {
  ...
  x1 = bx - lmargin; x2 = x1 + table->bwidth;
  y1 = by;  y2 = y1 + table->bheight;
  
  if (y2 >= ystart && y1 < *ytop)
  {
   PSprintf("gsave 0.2 setlinewidth\n");
   
   if (y1 >= ystart && y1 < *ytop)
    PSprintf("%d %d M %d 0 RL stroke\n", 
	     x1, -(y1 - PS_page_offset - PS_start_y), (x2 - x1));
   
   if (y2 >= ystart && y2 < *ytop)
    PSprintf("%d %d M %d 0 RL stroke\n", 
	     x1, -(y2 - PS_page_offset - PS_start_y), (x2 - x1));
   
   y1 = MAX(y1, ystart) - PS_page_offset - PS_start_y;
   y2 = MIN(y2, *ytop) - PS_page_offset - PS_start_y;
   
   PSprintf("%d %d M 0 %d RL stroke\n", x1, -y1, -(y2 - y1));
 ...
 for(y = 0; y < table->numRows; y++)
 {
  for(x = 0; x < table->numColumns; x++)
  {
   ...
   do
   {
    if (rec->y < ystart && rec->type != E_TABLE)
    {
     if (rec == field->last_rec) break;
     rec = rec->next;
     continue;
    ...
    if (rec->y > *ytop) 
    {
     if (rec->y < maxheight) maxheight = rec->y;
     np = 1;
     break;
    }
    ...
    if (rec->type == E_TABLE)
    {
     ...
     recin = TRUE;
     np |= PStableRange(hw, rec, fontfamily, xpos, ystart, &maxheight,
			lmargin, footnotes_this_page, footnotes_this_line,
			reserved_space);
     if (maxh < maxheight) maxheight = maxh;
     if (np) break;
    }
    else
    {
     if (rec->y + rec->line_height > *ytop) 
     { 
      if (rec->y < maxheight) maxheight = rec->y;
      np = 1;
      break;
     ...
     else
     {
      ...
      recin = TRUE;
      PSprintEleRec(hw, rec, fontfamily, xpos, ypos, lmargin,
		    footnotes_this_page, footnotes_this_line,
		    reserved_space);
     }
    }
   ...
   if (recin && table->borders && !field->contVert && !field->contHoriz)
   {
    int cwidth, cheight;
    int i, x1, x2, y1, y2;
    
    cwidth = field->colWidth;
    cheight = field->rowHeight;
    for(i = x + 1; i < x + field->colSpan; i++)
    {
     cwidth += table->table[y * table->numColumns + i].colWidth;
    }
    for(i = y + 1; i < y + field->rowSpan; i++)
    {
     cheight += table->table[i * table->numColumns + x].rowHeight;
    }
    
    x1 = bx + table->borders + 1 - lmargin;
    x2 = x1 + cwidth - table->borders - 1;
    y1 = by + table->borders + 1;
    y2 = y1 + cheight - table->borders - 1;
  ...
  bx = eptr->x;
  by += table->table[y * table->numColumns].rowHeight;
 }
 if (np)
  *ytop = maxheight;
...
*	5	option menu
*/
static void PSwidget(hw, eptr, fontfamily)
HTMLWidget hw;
struct ele_rec *eptr;
int fontfamily;
{
...
*
*/ 
static int PSrle_encode(scanline, rleline, wide)
...
* 
*/
static void PScolormap(color, nc, rmap, gmap, bmap)
int color;
int nc;
int *rmap;
int *gmap;
int *bmap;
{
 ...
 {
  if (color)  
   PSprintf("%02x%02x%02x ", (rmap[i]>>8)&255, (gmap[i]>>8)&255,
	    (bmap[i]>>8)&255);
  else  
   PSprintf("%02x ", MONO((unsigned short)rmap[i], 
			  (unsigned short)gmap[i],
			  (unsigned short)bmap[i]));
...
* 
*/
static void PSrle_cmapimage(color)
int color;
{
 static char *txt[] =
 {
...
* Write the given image array 'pic' (B/W stippled, 1 byte per pixel,
				     ...
				     *
				     */
static int PSwrite_bw(pic, w, h, flipbw)
unsigned char *pic;
int w;
int h;
int flipbw;
 ...
 */
 ...
 static void PSimage(hw, eptr, fontfamily)
  HTMLWidget hw;
 struct ele_rec *eptr;
 int fontfamily;
{
...
#define Is_fg(i,n) \
  ...
  if (imgp == NULL || HTML_Draft_Mode) 
  {
    ...
    PSprintf("gsave currentpoint %d sub translate", h);
    if (anchor && eptr->bwidth)
      PSprintf(" %d 0 translate", eptr->bwidth);
    else
  ...
  if (anchor && eptr->bwidth)
    PSprintf("%d 0 translate\n", eptr->bwidth);
  else
    PSprintf("0 2 translate\n");
  PSprintf("%d -%d scale\n", w , h);
  
  F(imgp).ps_print(imgp, PSprintf, HTML_Color_Mode);
  ...
  if (((ncolors == 2) &&
       ((Isgray(img,0) && Isgray(img,1)) || (Is_bg(img,0) && Is_fg(img,1)) 
	|| (Is_fg(img,0) && Is_bg(img,1)) ))
      || ((ncolors == 1) && (Isgray(img,0) || Is_bg(img,0) || Is_fg(img,0))))
  {
 ...
 else
 {
  ...
  if (!rleline) 
  {
   Xc_WARNING(("failed to malloc space for rleline"));
   return;
  }
...
static void PSprintEleRec(hw, eptr, fontfamily, xpos, ypos, lmargin,
			  footnotes_this_page, footnotes_this_line,
			  reserved_space)
HTMLWidget hw;
struct ele_rec *eptr;
...
int reserved_space;
{
 ...
 {
 case E_TEXT:
  PStext(hw, eptr, fontfamily, eptr->edata);
  break;
 ...
 case E_TABLE:
  {
   ...
   do
   {
    ytop = ypos + Pixels_Page - (ypos - PS_start_y + reserved_space);
    if (HTML_Print_Footers)
    {
     footn = PStableCheckFootnotes(hw, eptr, ypos, ytop);
     *footnotes_this_line += footn;
     reserved_space = 
      ((footnote_space + (footnote_ptsize * 
			  (*footnotes_this_page + *footnotes_this_line)))
       / Points_Pixel);
     ytop = ypos + Pixels_Page - (ypos - PS_start_y + reserved_space);
    }
    
    np = PStableRange(hw, eptr, fontfamily, xpos, ypos, &ytop, lmargin, 
		      footnotes_this_page, footnotes_this_line,
		      reserved_space);
    if (np)
    {
     PS_start_y = ypos = ytop;
     PSshowpage();
     PSnewpage();
     *footnotes_this_page = *footnotes_this_line = 0;
     reserved_space = 0;
    }
    
   } while(np);
   
   while(eptr->next && eptr->next->line_number == line)
    eptr = eptr->next;
...
String ParseTextToPSString(hw, elist, startp, endp, start_pos, end_pos,
			   space_width, lmargin, fontfamily, url, time_str)
HTMLWidget hw;
...
char *time_str;
{
 ...
 int			old_width, tmp_width;
  
 HTML_Print_Headers = (int)(GlobPrintParam.bBanners);
 HTML_Print_Footers = (int)(GlobPrintParam.bFootnotes);
    
 HTML_Draft_Mode = (int)(GlobPrintParam.bDraft);
 HTML_Color_Mode = (int)(GlobPrintParam.bColor);
 ...
 */
...
#ifdef Xc_XMAYDAY
 page_dimens.page_width = 
  (double)(GlobPrintParam.page_format->width) / (double)COORD_SCALEVALUE;
 page_dimens.page_height = 
  (double)(GlobPrintParam.page_format->height) / (double)COORD_SCALEVALUE;
  
 page_dimens.top_margin = 20 * MM;
  
 factor = page_dimens.page_height / (double)(297 * MM);
  
 page_dimens.top_margin = page_dimens.bot_margin = 
  page_dimens.left_margin = page_dimens.right_margin = (20 * MM) * factor;  
#else
 ...
 page_dimens.text_height = (page_dimens.page_height - page_dimens.top_margin 
 ...
 */
 Points_Pixel = 1.0;		/* GetDpi(hw) / 72.0;*/
 ...
 pagewidth =  tmp_width = 
  (int)(((float)(210 - 40 )* MM) / Points_Pixel);
 old_width = hw->html.view_width;

 tmp_width -= lmargin;
 lmargin = 0;

 margin_width = hw->html.margin_width;
 hw->html.margin_width = 0;
 FormatAll(hw, &tmp_width);
#else
#ifdef OLD
 ...
 */
 ...
 Points_Pixel = Points_Pixel * 
  ((double)page_dimens.text_width / ((double)(210 - 40) * MM)); 
  
 Pixels_Page = (int) (page_dimens.text_height / Points_Pixel);		
  
#ifdef Xc_XMAYDAY
 PScalculFont(hw, fontfamily);
#endif
 PSinit();
  ...
  */
  ...
  if (line != eptr->line_number) 
  {
   ...
   if ((ypos - PS_start_y ) + height + reserved_space > Pixels_Page) 
   {
    PS_start_y = ypos;
    PSshowpage();
    PSnewpage();
  ...
  PSprintEleRec(hw, eptr, fontfamily, xpos, ypos, lmargin, 
		&footnotes_this_page, &footnotes_this_line, reserved_space);
...
#ifdef Xc_XMAYDAY
 hw->html.margin_width = margin_width;
 FormatAll(hw, &old_width);
#endif
