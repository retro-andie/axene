/*
** HG_Surface.c for XQuad in High_Graph/
...
static void dest();
static void *copy();
static void regenerate ___PROTO((c_HG_Surface *This, bbox_t bbox,
				 c_HG_Axe *HG_Axe));
static void change_xinfo ___PROTO((c_HG_Surface *This));
static error change_base ___PROTO((c_HG_Surface *This));
static void display_redraw ___PROTO((c_HG_Surface *This));
static void add_to_vectorgraph ___PROTO((c_HG_Surface *This));
static void sub_to_vectorgraph ___PROTO((c_HG_Surface *This));
static void setHG ___PROTO((c_HG_Surface *This, ...));
static void vset();
static void calcul_surface ___PROTO((c_HG_Surface *This));
static void ps_print ___PROTO((c_HG_Surface *This, c_PostScript *post));
static boolean readHG ___PROTO((c_DocFile *doc,char *keyword,
				long param, BaseStd_t *base));
static boolean writeHG ___PROTO((c_HG_Surface *This, c_DocFile *doc));
     
sf_HG_Surface fc_HG_Surface =
{
...
int		sub_class;
{
  ...
  if ( (This = Xc_malloc("HG_Surface", sizeof(c_HG_Surface)) ) == NULL)
    return NULL;
  This->f = &fc_HG_Surface;
  
  This->highg = highg;
  This->class = HG_SURFACE;
...
c_HG_Surface	*This;
{
  ...
  if (This->nb_surface)
  {
    for(i=0; i<This->nb_surface; i++)
    {
      if (This->ordonnee)
	BG_DELETE(This->ordonnee[i])(This->ordonnee[i]);
      BG_DELETE(This->polygone[i])(This->polygone[i]);
    }
    if (This->ordonnee)
      Xc_free(This->ordonnee);
    Xc_free(This->polygone);
  }
  if (This->nb_abscisse)
  {
...
BaseStd_t	*base;
{
  c_HG_Surface	*hg_copy;
  
  if ( (hg_copy = Xc_malloc("HG_Surface", sizeof(c_HG_Surface)) ) == NULL)
    return NULL;
  memcpy(hg_copy, This, sizeof(c_HG_Surface));
...
c_HG_Axe	*HG_Axe;
{
  ...
  This->graduation = (c_BG_Line **)Xc_malloc("surface grad **", 
					    sizeof(c_BG_Line *)*nbs);
  This->string = (c_BG_Text **)Xc_malloc("surface string **", 
  ...
  This->nb_surface = nbc = highg->nb_ordonnee;
  if (highg->ordonnee && highg->d_ordonnee)
    This->ordonnee = (c_BG_Text **)Xc_malloc("surface ord **", 
					     sizeof(c_BG_Text *) * nbc);
  This->polygone = (c_BG_Polygone **)Xc_malloc("surface poly **", 
					       sizeof(c_BG_Polygone *) * nbc);
  totaux = (real *)Xc_malloc("totaux", sizeof(real) * nbs);
  for(j=0; j<nbs; j++) totaux[j] = 0.0;
  ...
  for(i=0; i<nbc; i++)
  {
    ...
    This->polygone[i] = (c_BG_Polygone *)NEW(c_BG_Polygone)(_BaseStd);
    BG_SET(This->polygone[i])(This->polygone[i],
			      XcBG_Polygone_INS_FG_COLOR, color,
			      XcBG_Polygone_END);
    point1.dx = bbox.llx;
    for(j=0; j<nbs; j++)
    {
      ...
      BG_SET(This->polygone[i])(This->polygone[i],
				XcBG_Polygone_ADD_POINT, point1,
				XcBG_Polygone_END);
      totaux[j] += highg->tab_value[i][one_absc?0:j];
      point1.dx += size.dx;
    }
  ...
  for(i=0; i<This->nb_surface; i++)
  {
    if (This->ordonnee)
      BG_CHANGE_XINFO(This->ordonnee[i])(This->ordonnee[i], 
					 &This->highg->X_info);
    BG_CHANGE_XINFO(This->polygone[i])(This->polygone[i], 
				       &This->highg->X_info);
  }
  ...
  for(i=0; i<This->nb_surface; i++)
  {
    if (This->ordonnee)
      BG_DISPLAY_REDRAW(This->ordonnee[i])(This->ordonnee[i], highg->matrix,
					   highg->scale, 0, 0);
    BG_DISPLAY_REDRAW(This->polygone[i])(This->polygone[i], highg->matrix,
					 highg->scale, 0, 0);
  }
  for(i=0; i<This->nb_abscisse; i++)
  {
  ...
  for(i=0; i<This->nb_surface; i++)
  {
    if (This->ordonnee)
      BG_ADD_TO_VECTORGRAPH(This->ordonnee[i])(This->ordonnee[i], 
					       This->highg->vectorg);
    BG_ADD_TO_VECTORGRAPH(This->polygone[i])(This->polygone[i], 
					     This->highg->vectorg);
  }
  for(i=0; i<This->nb_abscisse; i++)
  {
  ...
  for(i=0; i<This->nb_surface; i++)
  {
    if (This->ordonnee)
      BG_SUB_TO_VECTORGRAPH(This->ordonnee[i])(This->ordonnee[i], 
					       This->highg->vectorg);
    BG_SUB_TO_VECTORGRAPH(This->polygone[i])(This->polygone[i], 
					     This->highg->vectorg);
  }
  for(i=0; i<This->nb_abscisse; i++)
  {
...
static void setHG(c_HG_Surface *This, ...)
{
  va_list ap;
...
va_list ap;
{
  ...
  do {
    switch(va_arg(ap, HG_Surface_set_code_t))
    {
    case XcHG_Surface_END:
      Xc_TRACE(("XcHG_Surface_END"));
      out_flag = TRUE;
      break;
    default:
      Xc_BREAK(("Unknow HG_Surface_set_code"));
      break;
    }
  } while(!out_flag);
  if (calcul)
...
static void calcul_surface(This)
c_HG_Surface	*This;
{
}

/* -------------------------------------------------------------------- **
** ps print, read & write HG_Surface					**
** -------------------------------------------------------------------- */ 
