...
static void dest();
static void *copy();
static void regenerate ___PROTO((c_HG_Histo *This, int ordonnee, 
				 bbox_t	bbox, c_HG_Axe *HG_Axe));
static void change_xinfo ___PROTO((c_HG_Histo *This));
static error change_base ___PROTO((c_HG_Histo *This));
...
static boolean writeHG ___PROTO((c_HG_Histo *This, c_DocFile *doc));
     
sf_HG_Histo fc_HG_Histo =
{
...
int		sub_class;
{
  ...
  if ( (This = Xc_malloc("HG_Histo", sizeof(c_HG_Histo)) ) == NULL)
    return NULL;
  This->f = &fc_HG_Histo;
  
  This->highg = highg;
  This->class = HG_HISTOGRAMME;
...
BaseStd_t	*base;
{
  ...
  if ( (hg_copy = Xc_malloc("HG_Histo", sizeof(c_HG_Histo)) ) == NULL)
    return NULL;
  memcpy(hg_copy, This, sizeof(c_HG_Histo));
...
c_HG_Axe	*HG_Axe;
{
  ...
  if (highg->ordonnee && highg->d_ordonnee)
  {
    ...
    origin.dx = bbox.llx;
    origin.dy = bbox.ury + SCALE_FROM_MILLIMETERS(1.5);
    BG_SET(This->ordonnee)
      (This->ordonnee,
       ...
       XcBG_Text_RULER, XqR_H_ALIGN_CENTER|XqR_V_ALIGN_CENTER|XqR_MULTILINE,
       XcBG_Text_END);
  }
  ...
  This->nb_histo = nbs = highg->nb_abscisse;
  This->box = (c_BG_Box **)Xc_malloc("histo box **", 
				     sizeof(c_BG_Box *)*nbs);
  This->string = (c_BG_Text **)Xc_malloc("histo string **", 
					sizeof(c_BG_Text *)*nbs);
  
  size.dx = width / (nbs + 1);
  origin.dx = bbox.llx + size.dx / 2;
  origin.dy = F(HG_Axe).convert_value(HG_Axe, 0.0);
  
  t_orig.dy = point1.dy + SCALE_FROM_MILLIMETERS(1); 
  ...
  for(i=0; i<highg->nb_abscisse; i++)
  {
    ...
    size.dy = F(HG_Axe).convert_value(HG_Axe, highg->tab_value[ordonnee][i]) 
      - origin.dy;
    ...
    t_orig.dx = origin.dx + size.dx / 2 - tstyle->point_size / 4;
...
va_list ap;
{
  ...
  do {
    switch(va_arg(ap, HG_Histo_set_code_t))
    {
    case XcHG_Histo_END:
      Xc_TRACE(("XcHG_Histo_END"));
      out_flag = TRUE;
      break;
    default:
      Xc_BREAK(("Unknow HG_Histo_set_code"));
      break;
    }
  } while(!out_flag);
  if (calcul)
...
static void calcul_histo(This)
c_HG_Histo	*This;
{
}

/* -------------------------------------------------------------------- **
** ps print, read & write HG_Histo					**
** -------------------------------------------------------------------- */ 
