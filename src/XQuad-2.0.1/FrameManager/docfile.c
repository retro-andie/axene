/*
** docfile.c for XQuad in FrameManager/
**
...
** Last update Wed Sep 18 01:59:30 1996 Emmanuel Paris
*/
...
** ----------------------------------------------------------------- */
static boolean cb_frame(doc, keyword, param, feuille)
c_DocFile *doc;
char *keyword;
long param;
c_Feuille *feuille;
{
  ...
  if(FUNCTION(c_Cadre, read)(doc, keyword,
			     feuille->sFactor, feuille->BaseStd) == FALSE)
    return FALSE;
  ...
  if(frame->object_type == CADRE_VECTOR)
	...
	F(graph).init(graph, feuille->moteur);
      Xc_TRACE(("init ok"));
    }
