  ...
  /* x est ici la matrice de GRAM des bi.  */
{
  ...
  do
  {
	...
	if(cmpt<3){x=xinit;prec=(prec<<1)-2;goto LABLLLGRAM;}
	  ...
	  {
	    if(DEBUGLEVEL) outerr(x);err(lllger3);
	  }
#endif
      }
    }
...
*/
    long npass = 0;
    long ltop3 = avma; /* Excludes region with tmat1 and matmid */
	...
	{
          long jcol = 1 + ((icol + ijdif - 1) % ncol);
          long previous_avma = avma;
          long k1 = (gcmp(gcoeff(dotprd, icol, icol),
		                gcoeff(dotprd, jcol, jcol) ) > 0)
				? icol : jcol;	/* index of larger column */
	  long k2 = icol + jcol - k1;	/* index of smaller column */
	  GEN q;
...
long prec;
{
  ...
  w=cgetg(nv,17);
  for(j=1;j<nv;j++)
    w[j]=lmul(gmul(gtrans((GEN)mres[j]),t2),(GEN)mres[j]);
  per=indexsort(w);
  for(j=1;j<nv;j++)
  {
