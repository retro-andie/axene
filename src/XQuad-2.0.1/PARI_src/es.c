  ...
  if (len)
    {
      ...
      save = len;
      len--;
      while ((pariOutStr.s[len] == ' ') || (pariOutStr.s[len] == '0'))
	len--;
      if (pariOutStr.s[len] == '.')
	len--;
      strcpy(pariOutStr.s+len+1, pariOutStr.s+save);
    }
  return pariOutStr.s;
}
...
long dec;
{
  ...
  if (len)
	...
	len--;
      if (pariOutStr.s[len] == '.')
	len--;
      if ((len == 0) && (pariOutStr.s[len] == '0'))
	strcpy(pariOutStr.s, "0");
      else
	strcpy(pariOutStr.s+len+1, pariOutStr.s+save);
    }
  return pariOutStr.s;
...
char c;
{
  if ((errfile==outfile? pariErr && pariErr->fputc: pariOut && pariOut->fputc)) {
    (errfile==outfile? pariErr->fputc: pariOut->fputc)(c);
    return;
  }
  putc(c, outfile);
...
char *s;
{
  if ((errfile==outfile? pariErr && pariErr->fputs: pariOut && pariOut->fputs)) {
    (errfile==outfile? pariErr->fputs: pariOut->fputs)(s);
    return;
  }
  fputs(s, outfile);
...
void
ecrireall(x, format, dec, chmp, flbl)
GEN x;
char format;
long dec;
long chmp;
	...
	/* ecriture d'un entier */
  {
	      ...
	      /*  reel 0 */
	{
	  ...
	  zeros(dec);
	      ...
	      /* reel non nul */
	{
	  if(expo(x)>=((lg(x)-2)<<TWOPOTBITS_IN_LONG))
...
unsigned int n;
{
  ...
  sprintf(thestring, " Occupation of the MathsLib stack : %6.2f percent\n",r); pariputs(thestring);
...
isone(g)
GEN g;
/* renvoie 1 ou-1 si g est 1 ou-1,0 sinon */
{
  long i,sig;
  switch (typ(g))
  {
...
isfactor(g)
GEN g;
/* si g est un monome,renvoie son signe,0 sinon */
{
  long i,deja=0,sig=1;
  switch(typ(g))
  {
...
static void
brutiall(g, format, dec, sanssigne, flbl)
GEN g;
char format;
long dec;
long sanssigne;
...
static void
matbruti(g, format, dec, sanssigne)
GEN g;
char format;
long dec;
long sanssigne;
...
void
bruterr(x, format, dec)
GEN x;char format;long dec;
{
  FILE *tmpfile;
...
{
/*pariputs("            GP/PARI CALCULATOR Version 1.39.00\n");*/
