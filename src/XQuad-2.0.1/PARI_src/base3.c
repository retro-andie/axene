...
# include "genpari.h"
GEN nfreducemat ___PROTO((GEN nf, GEN a, GEN b, GEN c, GEN d));
GEN nfgauss ___PROTO((GEN nf, GEN a, GEN b, GEN prhall));
GEN fasthnf ___PROTO((GEN x,GEN detmat));
GEN nfreducemodpr ___PROTO((GEN nf, GEN x, GEN prhall));
GEN nfbezout ___PROTO((GEN nf, GEN a, GEN b, GEN ida, GEN idb, GEN *u, GEN *v, GEN *w, GEN *di));
/*******************************************************************
 ...
 d'entiers et ressort son inverse sous forme d'un vecteur */
{
  ...
  if(isnfscalar(x))
  {
    z=cgetg(N+1,17);z[1]=(long)ginv((GEN)x[1]);
...
GEN x;
{
  ...
  if((typ(x)<=10)||(typ(x)==18))
  {
    ...
    return gerepile(av,tetpil,idealmul(nf,x,id));
  }
  ...
  if(lg(x)<=N) {tetpil=avma;return gerepile(av,tetpil,idealmul(nf,x,id));}
...

{
  ...
  if((typ(b)<=10)||(typ(a)==18)) b=principalideal(nf,b);
  if((typ(b)!=19)||(lg(b)!=2))
    err(talker,"incorrect type in idealhermite2");
...

{
  ...
  rx=lg(x)-1;ry=lg(y)-1;
  if((!rx)||(!ry)) {tetpil=avma;y=gscalmat(gzero,lgef((GEN)nf[1])-3);}
  else
  {
...
GEN
ideallllredall(nf, ix, vdir, prec, precint)
GEN nf;
GEN ix;
GEN vdir;
long prec;
...
GEN
ideallllredpart1(nf, x, vdir, flprem, prec)
GEN nf;
GEN x;
GEN vdir;
long flprem;
...
GEN
ideallllredpart1spec(nf, x, matt2, flprem, prec)
GEN nf;
GEN x;
GEN matt2;
long flprem;
...
GEN
ideallllredpart2(nf, arch, gamma, prec)
GEN nf;
GEN arch;
GEN gamma;
long prec;
...
GEN
newidealcoprimeinvabc(nf, x, a, b, c)
GEN nf;
GEN x;
GEN a;
GEN b;
...
GEN
element_mulvecrow(nf, x, m, i, lim)
GEN nf;
GEN x;
GEN m;
long i;
...
#define element_powmodpr(nf,x,k,prhall) (nfreducemodpr(nf,element_pow(nf,x,k),prhall))

GEN
element_divmodpr(nf, x, y, prhall)
GEN nf;
GEN x;
GEN y;
GEN prhall;
...
GEN
nfbezout(nf, a, b, ida, idb, u, v, w, di)
GEN nf;
GEN a;
GEN b;
GEN ida;
