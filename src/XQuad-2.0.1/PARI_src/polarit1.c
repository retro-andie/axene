...
GEN gnorml1 ___PROTO((GEN x, long PREC)),laguer ___PROTO((GEN pol,long N,GEN y0,GEN EPS,long PREC));
GEN square_free_factorization ___PROTO((GEN pol));
...
GEN p;
{
  ...
  GEN   xmod,u,v,w,a0,polb,pold,polu,polt,puix,vker,vran,zmodp,unmodp;
  GEN   p1,p2,Q;
  
  if((typ(f)!=10)||gcmp0(f)||(typ(p)!=1)) err(factmoder);
  if(!cmpis(p,2)) return factmod(f,p);
  if((lgef(p)>3)||((lgef(p)==3)&&(cmpis(p,VERYBIGINT)>0))) 
    return factmod_gen(f,p);
  else
  {
    a0=(GEN)(f[2]);vf=varn(f);
    if((typ(a0)!=3)||!gegal((GEN)a0[1],p))
    {p=gcopy(p);av=avma;unmodp=gmodulcp(gun,p);f=gmul(f,unmodp);}
    else {av=avma;unmodp=gmodulcp(gun,p);}
...
/*
  ...
  */
	  ...
	  {
	    vran=cgetg(r+1,18);
	    for(i=1;i<=r;i++) vran[i]=lstoi(mymyrand()%psim);
	    polt=gmul(vker,vran);
	    for(i=1;(i<=kk)&&(kk<r);i++)
	    {
...
long r;
{
/* a usage interne. Pas de verifs. On suppose que f est un polynome a coeffs
   dans Z de degre n ayant n racines distinctes nod p, et p>2, r>=2. On
   rend les n racines p-adiques en precision une puissance de 2>=r.*/

  long av=avma,tetpil,i,e,e1,n;
  GEN fa,fp,y,yi,p1;

  fa=rootmod(f,p);n=lgef(f)-3;fp=deriv(f,varn(f));y=cgetg(n+1,17);
  for(i=1;i<=n;i++)
  {
    ...
    do
    {
      ...
      e=e1;if(e<r) {e1<<=1;yi=gprec(yi,e1);}
    }
    while(e<r);
    y[i]=(long)yi;
  }
...
GEN
nilordpadic(p, r, fx, mf, gx)
GEN p;long r;GEN fx;long mf;GEN gx;
{
...
GEN
Decomppadic(p, r, f, mf, theta, chi, nu)
GEN p;long r;GEN f;long mf;GEN theta;GEN chi;GEN nu;
{
...
GEN
laguer(pol, N, y0, EPS, PREC)
GEN pol;long N;GEN y0;GEN EPS;long PREC;
{
...
GEN x;long PREC;
{
  ...
  switch(typ(x))
  {
    ...
    default: err(talker,"not a MathLib object in gnorml1");
      return gnil;
  }
}
