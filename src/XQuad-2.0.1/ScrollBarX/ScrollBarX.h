/*
** ScrollBarX.h for XQuad in ScrollBarX/
...
** Last update Sat Jan  8 16:08:23 2000 Emmanuel Paris
*/
...
typedef struct
{
 ...
 int		Wsize;		/* World size (COLUMN_MAX or ROW_MAX) */
 int		Msize;		/* min size, Rsize >= Msize */
 int		Rsize;		/* Real size */
 int		Vsize;		/* Viewable size (nb_colum or nb_row) */
 
 int		Bsize;		/* Bar size in pixel */
 int		Csize;		/* Cursor size = Vsize*2/Rsize in pixel */
 
 int		CPos;		/* Cursor position in pixel */
 int		RPos;		/* Real position */
 
 int		PosX;		/* pos of the window in her parent */
 int		PosY;
 int		Size;		/* total window size in pixel */
...
#define DECALAGE                2
#define SCROLLB_INCR		1
#define SCROLLC_MINSIZE		7
