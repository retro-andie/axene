/*
** BG_Polygone.c for XQuad in Basic_Graph/
**
...
static void dest();
static void *copy();
static void change_xinfo ___PROTO((c_BG_Polygone *This, x_info *xinfo));
static error change_base ___PROTO((c_BG_Polygone *This, BaseStd_t *new_base));
static void display_redraw ___PROTO((c_BG_Polygone *This, c_Matrix *matrix,
				     coord_t scale, int dx, int dy));
static void display_handles ___PROTO((c_BG_Polygone *This, c_Matrix *matrix,
				      coord_t scale, int dx, int dy));
static void add_to_vectorgraph ___PROTO((c_BG_Polygone *This, 
					 c_VectorGraph *vectorg));
static void sub_to_vectorgraph ___PROTO((c_BG_Polygone *This, 
					 c_VectorGraph *vectorg));
static bbox_t *get_bounding_box ___PROTO((c_BG_Polygone *This));
static boolean is_in ___PROTO((c_BG_Polygone *This, coord_t x, coord_t y));
static void setBG ___PROTO((c_BG_Polygone *This, ...));
static void vset();
static void calcul_polygone ___PROTO((c_BG_Polygone *This));
static boolean ps_print ___PROTO((c_BG_Polygone *This, c_PostScript *post));
static boolean readBG ___PROTO((c_DocFile *doc,char *keyword,
				long param, BaseStd_t *base));
static boolean writeBG ___PROTO((c_BG_Polygone *This, c_DocFile *doc));

sf_BG_Polygone fc_BG_Polygone =
{
  cons, dest, copy,
...
BaseStd_t	*base;
{
  ...
  if ( (This = Xc_malloc("BG_Polygone", sizeof(c_BG_Polygone)) ) == NULL)
    return NULL;
  This->f = &fc_BG_Polygone;
  
  This->type = BG_POLYGONE;
  This->BaseStd = base;
...
BaseStd_t	*base;
{
  c_BG_Polygone	*bg_copy;
  
  if ( (bg_copy = Xc_malloc("BG_Polygone", sizeof(c_BG_Polygone)) ) == NULL)
    return NULL;
  memcpy(bg_copy, This, sizeof(c_BG_Polygone));
...
int		dy;
{
  ...
  xpoint = (XPoint *)Xc_malloc("xpoints", sizeof(XPoint)*(This->nb_point+1));
  ...
  if (!This->ins_fg_color->transparency && This->nb_point > 2)
	...
	val_gc.fill_style = FillOpaqueStipple;
      ...
      XChangeGC(display, gc, GCFunction | GCForeground | GCBackground | 
		GCFillRule /* | GCFillStyle */ ,&val_gc);
      
      XFillPolygon(display, window, gc, xpoint, This->nb_point, Complex,
		   CoordModeOrigin); 
    }
  ...
  if (This->line_thickness)
    {
      ...
      xpoint[This->nb_point].x = xpoint[0].x;
      xpoint[This->nb_point].y = xpoint[0].y;
      XDrawLines(display, window, gc, 
		 xpoint, This->nb_point+1, CoordModeOrigin);
    }
...
static void setBG(c_BG_Polygone *This, ...)
{
  va_list ap;
...
va_list ap;
{
  ...
  do {
    switch(va_arg(ap, BG_Polygone_set_code_t))
    {
    case XcBG_Polygone_ADD_POINT:
      Xc_TRACE(("XcBG_Polygone_POINT2"));
      calcul = TRUE;
      bg_point = This->list_point;
      if (bg_point)
	{
	  while(bg_point->NextPoint)
	    bg_point = bg_point->NextPoint;
	  bg_point->NextPoint = (BG_Point *)Xc_malloc("bg_point",
						      sizeof(BG_Point));
	  bg_point = bg_point->NextPoint;
	}
      else
	{
	  bg_point = This->list_point = 
	    (BG_Point *)Xc_malloc("bg_point", sizeof(BG_Point));
	}
      bg_point->point = va_arg(ap, vector_t);
      bg_point->NextPoint = NULL;
      This->nb_point = This->nb_point + 1;
      break;
    case XcBG_Polygone_ADD_POINTS:
      Xc_TRACE(("XcBG_Polygone_POINT2"));
      calcul = TRUE;
      nb_point = va_arg(ap, int);
      break;
    case XcBG_Polygone_INS_FG_COLOR:
      Xc_TRACE(("XcBG_Polygone_INS_FG_COLOR"));
      This->ins_fg_color = va_arg(ap, c_Color *);
      break;
    case XcBG_Polygone_INS_BG_COLOR:
      Xc_TRACE(("XcBG_Polygone_INS_BG_COLOR"));
      This->ins_bg_color = va_arg(ap, c_Color *);
      break;
    case XcBG_Polygone_OUT_FG_COLOR:
      Xc_TRACE(("XcBG_Polygone_OUT_FG_COLOR"));
      This->out_fg_color = va_arg(ap, c_Color *);
      break;
    case XcBG_Polygone_OUT_BG_COLOR:
      Xc_TRACE(("XcBG_Polygone_OUT_BG_COLOR"));
      This->out_bg_color = va_arg(ap, c_Color *);
      break;
    case XcBG_Polygone_DASHES:
      Xc_TRACE(("XcBG_Polygone_DASHES"));
      /* This->dashes = va_arg(ap, c_Dashes *); */
      break;
    case XcBG_Polygone_PATTERN:
      Xc_TRACE(("XcBG_Polygone_PATTERN"));
      /* This->pattern = va_arg(ap, c_Pattern *); */
      break;
    case XcBG_Polygone_LINE_THICKNESS:
      Xc_TRACE(("XcBG_Polygone_LINE_THICKNESS"));
      This->line_thickness = va_arg(ap, coord_t);
      break;
    case XcBG_Polygone_END:
      Xc_TRACE(("XcBG_Polygone_END"));
      out_flag = TRUE;
      break;
    default:
      Xc_BREAK(("Unknow BG_Polygone_set_code"));
      break;
    }
  } while(!out_flag);
  if (calcul)
...
static void calcul_polygone(This)
c_BG_Polygone	*This;
{
  int		i;
  BG_Point	*bg_point;
...
c_PostScript *post;
{
  ...
  if(this->list_point == NULL) return TRUE;
  ...
  if(post->ai_mode)
    {
      if(stroke && !F(post).putLine(post, "$p L", this->list_point->point.dx,
				    this->list_point->point.dy))
	return FALSE;
