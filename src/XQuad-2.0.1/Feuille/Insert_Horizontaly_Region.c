/*
** Insert_Horizontaly_Region.c for XQuad in Feuille/
** Insert region horizontaly
**
...
** Last update Fri Dec 13 18:38:44 1996 Emmanuel Paris
*/

#include "Select_Move_Resize_Cells.h"

void insert_horizontaly_region_init();
void insert_horizontaly_region_abort();
void insert_horizontaly_region_done();
void insert_horizontaly_region_to_xy();
void insert_horizontaly_region_trace();

void insert_horizontaly_region_init(cfeuille,xm,ym)
c_Feuille *cfeuille;
int	  xm;
int	  ym;
{
...
void insert_horizontaly_region_abort(cfeuille)
c_Feuille *cfeuille;
{ 
  DATA(Select_Move_Resize_Cell);
...
void insert_horizontaly_region_done(cfeuille)
c_Feuille *cfeuille;
{  
  int	incx;
  region_t region;
  ...
  if (cfeuille->cell_region->x_min == d(region).x_min)
    ...
    {
      incx = cfeuille->cell_region->x_min - d(region).x_min;
      d(region).x_min += incx;
      d(region).x_max += incx;
      region.x_min = d(region).x_min;
      region.x_max = cfeuille->cell_region->x_min - 1 + incx;
   }
  region.y_min = cfeuille->cell_region->y_min;
  region.y_max = cfeuille->cell_region->y_max;
  F(cfeuille->moteur).insert_zone_col(cfeuille->moteur, &region);
...
void insert_horizontaly_region_to_xy(cfeuille,xm,ym)
c_Feuille *cfeuille;
int	xm;
int	ym;
{ 
...
void insert_horizontaly_region_trace(cfeuille)
c_Feuille *cfeuille;
{  
  Display	*display;
  Window	window;
