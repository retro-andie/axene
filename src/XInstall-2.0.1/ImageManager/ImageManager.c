/*
** ImageManager.c for XInstall in ImageManager/
...
** Last update Tue Jan 20 16:05:26 1998 Robin Castanier
*/
...
static boolean restorePart();
static boolean putPart();
...
boolean		xshm_flag;
{
  ...
  
 {
  ...
  ximage = XGetImage
   (display, pixmap, 0, 0, 16, 16, (unsigned long)-1, XYPixmap);
  if (XGetPixel(ximage, 7, 9) != 1)
...
/* ----------------------------------------------------------------- ** 
** savePart - Save part of item                                      ** 
** ----------------------------------------------------------------- */
static boolean savePart(this, item, gc, x, y, w, h)
c_ImageManager *this;
image_item_t *item;
GC gc;
int x;
...
int h;
{
 ...
 switch(item->type)
 {
  /*--- XImage ---*/
 case XcPM_XIMAGE:
  item->save.data_ximage = (char *)Xc_malloc("save_img",
					     item->ximage->bytes_per_line *
					     item->ximage->height);
  memcpy(item->save.data_ximage, item->ximage->data, 
	 item->ximage->bytes_per_line * item->ximage->height);
  return TRUE;
  break;

  /*--- Pixmap ---*/
 case XcPM_PIXMAP:
  item->save.pixmap =  XCreatePixmap(this->display, 
				     DefaultRootWindow(this->display),
				     w, h, item->depth);
  if(item->depth == 1)
   XCopyPlane(this->display, item->pixmap, item->save.pixmap,
	      gc, x, y, w, h, 0, 0, 1L);
  else
   XCopyArea(this->display, item->pixmap, item->save.pixmap,
	     gc, x, y, w, h, 0, 0);
  return TRUE;
  break;
...
/* ----------------------------------------------------------------- ** 
** restorePart - Restore part of item                                ** 
** ----------------------------------------------------------------- */
static boolean restorePart(this, item, gc)
c_ImageManager *this;
image_item_t *item;
GC gc;
{
 ...
 switch(item->type)
 {
  /*--- XImage ---*/
 case XcPM_XIMAGE:
  memcpy(item->ximage->data, item->save.data_ximage,
	 item->ximage->bytes_per_line * item->ximage->height);
  Xc_free(item->save.data_ximage);
  return TRUE;
  break;
 ...
 case XcPM_PIXMAP:
  if(item->depth == 1)
   XCopyPlane(this->display,  item->save.pixmap, item->pixmap,
	      gc, 0, 0, 
	      item->save.w, item->save.h,
	      item->save.x, item->save.y, 1L);
  else
   XCopyArea(this->display, item->save.pixmap, item->pixmap,
	     gc, 0, 0,
	     item->save.w, item->save.h,
	     item->save.x, item->save.y);
  XFreePixmap(this->display, item->save.pixmap);
  return TRUE;
  break;
...
/* ----------------------------------------------------------------- ** 
** putPart - put part of item                                        ** 
** ----------------------------------------------------------------- */
static boolean putPart(this, item1, item2, gc, x, y, w, h, x_dest, y_dest)
c_ImageManager *this;
image_item_t *item1; /* src */
image_item_t *item2; /* dst */
GC gc;
int x;
int y;
int w;
int h;
{
  ...
  case XcPM_XIMAGE:
   {
    ...
    Xc_TRACE(("Ximage -> Ximage"));
    if (item2->ximage->depth != item1->ximage->depth)
    {
     return FALSE;
    }
    if (x_dest + w > item2->ximage->width)
     r_w = item2->ximage->width - x_dest;
    else
     r_w = w;
    if (y_dest + h > item2->ximage->height)
     r_h = item2->ximage->height - y_dest;
    else
     r_h = h;

    /* ZFormat */
    start = item1->ximage->data + y * item1->ximage->bytes_per_line +
     x * (item1->ximage->bits_per_pixel / 8);
    dest = item2->ximage->data + y_dest * item2->ximage->bytes_per_line +
     x_dest * (item2->ximage->bits_per_pixel / 8);
    while (r_h)
    {
     memcpy(dest, start, r_w * (item1->ximage->bits_per_pixel / 8));
     start += item1->ximage->bytes_per_line;
     dest += item2->ximage->bytes_per_line;
     r_h--;
    }
 ...
 case XcPM_PIXMAP:
  ...
  {
  case XcPM_XIMAGE:
   Xc_TRACE(("Pixmap -> Ximage"));
   return TRUE;
   break;
  case XcPM_PIXMAP:
   Xc_TRACE(("Pixmap -> Pixmap"));
   XCopyArea(this->display, item1->pixmap, item2->pixmap, gc,
	     x, y, w, h, x_dest, y_dest);
  default:
   return TRUE;
   break;
  }
