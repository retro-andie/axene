/*
** MainInterface.c for XMayday in MainInterface/
**
...
** Last update Thu Feb  3 16:31:59 2000 Emmanuel Paris
*/
...
#include "../Pixmaps/xmayday_icon.zpm"
...
{ 
#include "../Global/XMayday.ad.h"
 NULL
};

static char cryptString[] = "i(:^9Qb";
static char appName[] = { 'X' - 'i', 'M' - '(', 'a' - ':',
			   'y' - '^', 'd' - '9', 'a' - 'Q',  'y' - 'b', '\0'};
...
c_ManageWidget *ManageW;
{
 ...
 /*--- Application shell ---*/
#ifdef PL
 sprintf(title, "%s %d.%d.%d", appName, MAJOR, MINOR, PL);
#else
 sprintf(title, "%s %d.%d", appName, MAJOR, MINOR);
#endif
 nargs = 0;
 XtSetArg(argsw[nargs], XmNminWidth, 300); nargs++;
 XtSetArg(argsw[nargs], XmNminHeight, 200); nargs++;
 XtSetArg(argsw[nargs], XmNtitle, title); nargs++;
