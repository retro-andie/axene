...
void
png_save_uint_32(buf, i)  png_bytep buf; png_uint_32 i;
{
...
void
png_save_uint_16(buf, i)  png_bytep buf; png_uint_16 i;
{
...
void
png_write_uint_32(png_ptr, i)  png_structp png_ptr; png_uint_32 i;
{
 png_byte buf[4];
...
void
png_write_uint_16(png_ptr, i)  png_structp png_ptr; png_uint_16 i;
{
 png_byte buf[2];
...
/* Write a png chunk all at once.  The type is an array of ASCII characters
   ...
   functions instead.  */
void
png_write_chunk(png_ptr, type, data, length)  png_structp png_ptr; png_bytep type;
...
void
png_write_chunk_start(png_ptr, type, total_length)  png_structp png_ptr; png_bytep type;
...
void
png_write_chunk_data(png_ptr, data, length)  png_structp png_ptr; png_bytep data; png_uint_32 length;
{
...
void
png_write_chunk_end(png_ptr)  png_structp png_ptr;
{
 /* write the crc */
 png_write_uint_32(png_ptr, ~png_ptr->crc);
}
...
void
png_write_sig(png_ptr)  png_structp png_ptr;
{
...
void
png_write_IHDR(png_ptr, width, height, bit_depth, color_type, compression_type, filter_type, interlace_type)  png_structp png_ptr; png_uint_32 width; png_uint_32 height;
...
/* write the palette.  We are careful not to trust png_color to be in the
   ...
   structure. */
void
png_write_PLTE(png_ptr, palette, number)  png_structp png_ptr; png_colorp palette; int number;
{
 int i;
 png_colorp pal_ptr;
 png_byte buf[3];
...
/* write an IDAT chunk */
void
png_write_IDAT(png_ptr, data, length)  png_structp png_ptr; png_bytep data; png_uint_32 length;
{
...
/* write an IEND chunk */
void
png_write_IEND(png_ptr)  png_structp png_ptr;
{
...
/* write a gAMA chunk */
void
png_write_gAMA(png_ptr, gamma)  png_structp png_ptr; double gamma;
{
 png_uint_32 igamma;
 png_byte buf[4];
...
void
png_write_sBIT(png_ptr, sbit, color_type)  png_structp png_ptr; png_color_8p sbit; int color_type;
{
 png_byte buf[4];
 int size;
...
void
png_write_cHRM (png_ptr, white_x, white_y, red_x, red_y, green_x, green_y, blue_x, blue_y)   png_structp png_ptr; double white_x; double white_y;
...
void
png_write_tRNS(png_ptr, trans, tran, num_trans, color_type)  png_structp png_ptr; png_bytep trans; png_color_16p tran;
...
void
png_write_bKGD(png_ptr, back, color_type)  png_structp png_ptr; png_color_16p back; int color_type;
{
 png_byte buf[6];
...
/* write the histogram */
void
png_write_hIST(png_ptr, hist, number)  png_structp png_ptr; png_uint_16p hist; int number;
{
 int i;
 png_byte buf[3];
...
/* write a tEXt chunk */
void
png_write_tEXt(png_ptr, key, text, text_len)  png_structp png_ptr; png_charp key; png_charp text;
...
void
png_write_zTXt(png_ptr, key, text, text_len, compression)  png_structp png_ptr; png_charp key; png_charp text;
...
void
png_write_pHYs(png_ptr, x_pixels_per_unit, y_pixels_per_unit, unit_type)  png_structp png_ptr; png_uint_32 x_pixels_per_unit;
...
void
png_write_oFFs(png_ptr, x_offset, y_offset, unit_type)  png_structp png_ptr; png_uint_32 x_offset;
...
void
png_write_tIME(png_ptr, mod_time)  png_structp png_ptr; png_timep mod_time;
{
 png_byte buf[7];
...
void
png_write_start_row(png_ptr)  png_structp png_ptr;
{
 /* set up row buffer */
...
void
png_write_finish_row(png_ptr)  png_structp png_ptr;
{
 int ret;
...
/* pick out the correct pixels for the interlace pass.
   ...
   */
void
png_do_write_interlace(row_info, row, pass)  png_row_infop row_info; png_bytep row; int pass;
{
...
* chosen filter */
void
png_write_find_filter(png_ptr, row_info)  png_structp png_ptr; png_row_infop row_info;
{
 ...
 * that has been chosen, as it doesn't actually do anything to the data. */
     ...
     png_ptr->do_filter != PNG_FILTER_NONE)
 {
  ...
  for (i = 0, rp = row_buf + 1; i < (int)row_info->rowbytes; i++, rp++)
  {
   v = *rp;
   sum += (v < 128) ? v : 256 - v;
  }
 ...
 if (png_ptr->do_filter & PNG_FILTER_SUB)
 {
  ...
  for (lp = row_buf + 1; i < (int)row_info->rowbytes; i++, rp++, lp++, dp++)
  {
 ...
 if (png_ptr->do_filter & PNG_FILTER_UP)
 {
  ...
  for (i = 0, rp = row_buf + 1, dp = png_ptr->up_row + 1,
       pp = prev_row + 1; i < (int)row_info->rowbytes; i++, rp++, pp++, dp++)
  {
 ...
 if (png_ptr->do_filter & PNG_FILTER_AVG)
 {
  ...
  for (lp = row_buf + 1; i < (int)row_info->rowbytes;
       i++, rp++, pp++, lp++, dp++)
  {
 ...
 if (png_ptr->do_filter & PNG_FILTER_PAETH)
 {
  ...
  for (lp = row_buf + 1, cp = prev_row + 1; i < (int)row_info->rowbytes;
       i++, rp++, pp++, lp++, dp++, cp++)
  {
...
void
png_write_filtered_row(png_ptr, filtered_row)  png_structp png_ptr; png_bytep filtered_row;
{
