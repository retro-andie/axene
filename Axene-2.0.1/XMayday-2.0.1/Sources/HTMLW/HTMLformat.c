...
extern void TableCalculateDimensions();
extern void ComputeTableAlignment();
extern struct ele_rec *TableLocateElement();
extern void TableRefresh();
extern int NoBodyImages();
...
static int RAlignment;	/* Rule alignment */
static int Alignment;	/* current alignment */
...
int bw;
{
 ...
 if (eptr == NULL)
 {
  Xc_FATAL(("Cannot allocate space for element buffer"));
 }
 ...
 eptr->font = fp;
 eptr->alignment = ALIGN_BOTTOM | Alignment;
 eptr->selected = False;
  ...
  */
  ...
  {
   eptr->edata_len = 0;
   Xc_FATAL(("Cannot allocate space for copy of text element data"));
  }
  ...
  */
  ...
  if (edata != NULL)
  {
   ...
   {
    eptr->edata_len = 0;
    Xc_FATAL(("Cannot allocate space for copy of image element data"));
   }
   strcpy(eptr->edata, edata);
  }
  else
  ...
  */
   ...
   */
   if ((edata != NULL) && (strncmp(edata, INTERNAL_IMAGE,
   ...
   */
   if ((hw->html.delay_images == True) && (!internal))
   {
    /*
  ...
  */
  ...
  if (Current)			/* to avoid a bug when <TABLE> is the first tag */
  { 
   eptr->table_data = MakeTable (hw, edata, (x + IMAGE_DEFAULT_BORDER),
				 (y + IMAGE_DEFAULT_BORDER));
   eptr->next = NULL;
  }
  else
  {
  ...
  * Widget stuff
  */
  eptr->widget_data = MakeWidget(hw, edata, (x + IMAGE_DEFAULT_BORDER),
				 (y + IMAGE_DEFAULT_BORDER),
  ...
  */
  ...
  break;
 default:
  Xc_TRACE(("CreateElement:  Unknown type %d", type));
  eptr->ele_id = ElementId;
 ...
 */
 ...
 if (eptr == NULL)
 {
  Xc_FATAL(("SetElement: Error, setting a null element"));
 }
  ...
  */
   ...
   */
   if ((edata != NULL) &&
       (strncmp(edata, INTERNAL_IMAGE, strlen(INTERNAL_IMAGE)) == 0))
    internal = 1;
   else
    internal = 0;
  ...
  */
  ...
  eptr->table_data = MakeTable
   (hw, edata, (x + IMAGE_DEFAULT_BORDER), (y + IMAGE_DEFAULT_BORDER));
  eptr->next = NULL;
  break;
 case E_WIDGET:
  /*
  ...
  */
  ...
  break;
 default:
  Xc_TRACE(("SetElement:  Unknown type %d", type));
  eptr->ele_id = ElementId;
 ...
 */
 ...
 SetElement(hw, E_LINEFEED, currentFont, *x, *y, (char *)NULL,
	    NULL, NULL, IMAGE_DEFAULT_BORDER);
}
 ...
 */
 ...
 SetElement(hw, E_TEXT, currentFont, my_x, *y, buf, NULL,
	    NULL, IMAGE_DEFAULT_BORDER);
  
 Current->type = E_BULLET;
 Current->width = *x - my_x;
 AdjustBaseLine();
...
unsigned int width;
{
  ...
  */
   ...
   */
   ...
   if (currentFont->per_char[160-currentFont->min_char_or_byte2].width == 0)
   {
    memcpy(&(currentFont->per_char[160-currentFont->min_char_or_byte2]),
	   &(currentFont->per_char[32-currentFont->min_char_or_byte2]),
	   sizeof(XCharStruct));
    currentFont->per_char[160 - 
			  currentFont->min_char_or_byte2].attributes = 3141;
   }/* hack for lucida font, for non breakable space */
...
unsigned int width;
{
  ...
  */
   ...
   */
   ...
   if (((*x + all.width + MarginW) <= (int)width) || (nobreak))
   {
    char *tptr;
    int tlen;
    ...
    */
    ...
    XTextExtents(currentFont, tptr2, strlen(tptr2), &dir,
		 &ascent, &descent, &all);
  ...
  */
   ...
   * line break.
   */
   if ((*x + all.width + MarginW) <= (int)width)
   {
    if (line == NULL)
    {
     tlen = strlen(spc) + 1;
...
unsigned int width;
{
 ...
 Xc_TRACE(("TablePlace(hw,mptr,*x=%d,*y=%d,width = %d)", *x,*y,width));
 ...
 if (!(Current && Current->type == E_LINEFEED) && BaseLine != -100)
  LineFeed(hw, x, y); 
 recurse_table++;
 ...
 if (recurse_table == 0)
 {
  TableCalculateDimensions(hw, Current->table_data, *x, *y,
			   width - *x - hw->html.margin_width);
 ...
 */
 ...
 SetElement(hw, E_IMAGE, currentFont, *x, *y, tmpPtr, wTmp, hTmp,
	    border_width);
 ...
 */
 if ((Current->pic_data != NULL) && (!Preformat))
 {
  ...
  if (((*x + Current->pic_data->width + extra + MarginW) > (int)width) &&
      (Current->prev != NULL) &&
 ...
 */
 ...
 if (tptr != NULL)
 {
  ...
  do
  {
   if (caseless_equal_prefix(txt, "TOP", 3))
    Current->alignment = (Current->alignment & HALIGN_MASK) | ALIGN_TOP;
   else
   {
    if (caseless_equal_prefix(txt, "MIDDLE", 6))
     Current->alignment = (Current->alignment&HALIGN_MASK) | ALIGN_MIDDLE;
    else
     if (caseless_equal_prefix(txt, "BOTTOM", 6))
      Current->alignment = (Current->alignment&HALIGN_MASK) | ALIGN_BOTTOM;
   }
    
   if (caseless_equal_prefix(txt, "LEFT", 4))
    Current->alignment = (Current->alignment&VALIGN_MASK) | ALIGN_LEFT;
   else
   {
    if (caseless_equal_prefix(txt, "RIGHT", 5))
     Current->alignment = (Current->alignment&VALIGN_MASK) | ALIGN_RIGHT;
    else
     if (caseless_equal_prefix(txt, "CENTER", 6))
      Current->alignment = (Current->alignment&VALIGN_MASK) | ALIGN_CENTER;
   }
 ...
 */
 if (Current->pic_data != NULL)
 {
  ...
  if ((hw->html.border_images == True) ||
      ((Current->anchorHRef != NULL) && (!Current->pic_data->internal)))
  {
   if (Current->pic_data->delayed)
    extra = 4;
   else
  ...
  *x = *x + Current->pic_data->width + extra;
  ...
  if ((Current->alignment & VALIGN_MASK) == ALIGN_TOP)
  {
   Current->y_offset = 0;
  ...
  else if ((Current->alignment & VALIGN_MASK) == ALIGN_MIDDLE)
  {
   int baseline;
  ...
  Current->width = Current->pic_data->width + extra;
  Current->line_height = Current->pic_data->height + extra;
 ...
 */
 if ((Current->widget_data != NULL) && (!Preformat))
 {
  ...
  if (((*x + Current->widget_data->width + extra + MarginW) > (int)width) &&
...
XFontStruct *font;
{
 ...
 if (fptr == NULL)
 {
  Xc_WARNING(("No memory to expand font stack!"));
  return;
 }
...
static XFontStruct *PopFont()
{
 ...
 {
  Xc_TRACE(("Warning, popping empty font stack!"));
  font = FontStack->font;
 }
 return font;
}
 ...
 */
void TriggerMarkChanges(hw, mptr, x, y)
HTMLWidget hw;
struct mark_up **mptr;
int *x, *y;
{
  ...
  case M_NONE:
	...
	if (CurrentSelect->option_buf != NULL)
	{
	  ...
	  tptr = (char *)malloc(strlen(CurrentSelect->option_buf) +
				strlen((*mptr)->text) + 1);
  ...
  case M_BOLD:
    ...
    else
    {
      ...
      if (currentFont == hw->html.header1_font ||
	  currentFont == hw->html.header2_font ||
	  currentFont == hw->html.header3_font ||
	  currentFont == hw->html.header4_font ||
	  currentFont == hw->html.header5_font ||
	  currentFont == hw->html.header6_font)
	break;
  ...
  case M_ITALIC:
    ...
    else
    {
	  ...
	  currentFont == hw->html.header5_font ||
	  currentFont == hw->html.header6_font ||
	  currentFont == hw->html.big_font ||
	  currentFont == hw->html.small_font)
	break;
  ...
  case M_HEADER_6:
    ...
    break;
    /* alignment tags added by Xibios 2 from Axene */
  case M_LEFT:
    ConditionalLineFeed(hw, x, y, 1);
    Alignment = RAlignment = ALIGN_LEFT;
    break;
  case M_CENTER:
    ConditionalLineFeed(hw, x, y, 1);
    if (mark->is_end)
      ...
      Alignment = RAlignment = ALIGN_CENTER;
    break;
  case M_RIGHT:
    ConditionalLineFeed(hw, x, y, 1);
    if (mark->is_end)
      ...
      Alignment = RAlignment = ALIGN_RIGHT;
    break;
  case M_FONT:
#ifdef Xc_XMAYDAY
    if (mark->is_end)
    ...
    {
      tptr = ParseMarkTag(mark->start, MT_FONT, "COLOR");
      if (tptr)
      {
       int i;
       color_cells_t cell;
	...
	Fg = cell.pixel;
      ...
      PushFont(currentFont);
      tptr = ParseMarkTag(mark->start, MT_FONT, "SIZE");
      if (tptr)
      {
       int size;
   ...
   case M_ANCHOR:
    if (mark->is_end)
    {
     ...
     /*
     * Added by Xibios(2) from AXENE.
     * Add an empty ele_rec for managing empty anchor, ie:
     * <A NAME="label"></A>
     */
     if (Current && !Current->anchorName)
     {
      SetElement(hw, E_TEXT, currentFont, *x, *y, "", NULL,
		 NULL, IMAGE_DEFAULT_BORDER);
     }
     /* end of addition */
     AnchorText = NULL;
	...
	DashedUnderlines = hw->html.dashed_anchor_lines;
      ...
      */
      ...
      {
       if ((*(visitTestProc)(hw->html.previously_visited_test))(hw, tptr))
       {
	Fg = hw->html.visitedAnchor_fg;
	Underlines = hw->html.num_visitedAnchor_underlines;
	DashedUnderlines = hw->html.dashed_visitedAnchor_lines;
  ...
  case M_INPUT:
    if (CurrentForm != NULL)
    {
      ...
      if ((tptr != NULL) && (strcmp(tptr, "image") == 0))
      {
	free(tptr);
	tptr = (char *)malloc(strlen((*mptr)->start) + strlen(" ISMAP") +
			      strlen(MT_IMAGE) - strlen(MT_INPUT) + 1);
	...
	free(tptr);
      ...
      {
	if ((tptr != NULL) && (strcmp(tptr, "hidden") == 0))
	{
	  free(tptr);
	  WidgetId++;
  ...
  case M_INDEX:
    ...
    if (CurrentForm == NULL)
    {
      ...
      CurrentForm->enc_entity = 
	ParseMarkTag(mark->start, MT_INDEX, "ENCENTITY");
      CurrentForm->start = WidgetId;
      CurrentForm->end = -1;
	...
	(strlen("This is a searchable index.  Enter search keywords: ") + 1);
       ...
       */
      mark_tmp.start = 
	(char *)malloc(strlen("input SIZE=25 NAME=\"isindex\"") + 1);
...
/*
* Compute Align Line List
* recalcul x coordinate for all ele_rec element on all line
* take alignment field parameter
* Added by Xibios 2 from Axene
*/
void ComputeAlignLineList(hw, line_array, line_number, width)
HTMLWidget hw;
...
int width;
{
 ...
 for(i = 0; i < line_number; i++)
 {
  ...
  alignment = ALIGN_LEFT;
  while((eptr != NULL) && (eptr->line_number == i + 1))
  {
   if ((eptr->alignment & HALIGN_MASK) != alignment)
   {
    alignment = eptr->alignment & HALIGN_MASK;
    break;
   }
   eptr = eptr->next;
  }
  ...
  case ALIGN_CENTER:      
   eptr = line_array[i];
   while(eptr && eptr->type != E_LINEFEED)
    eptr = eptr->next;
      
   ewidth = hw->html.margin_width + eptr->x;
   x = (width - ewidth) / 2;
   if (x < 0)
    break;
      
   eptr = line_array[i];
   while(eptr && eptr->line_number == i + 1)
   {
    eptr->x += x;
    if (eptr->type == E_TABLE)
     ComputeTableAlignment(hw, eptr, x);
    eptr = eptr->next;
   }
   break;
  case ALIGN_RIGHT:
...
#ifndef NTRACE
 gettimeofday(&Tv, &Tz);
 Xc_TRACE(("FormatAll enter (%d.%d)", Tv.tv_sec, Tv.tv_usec));
#endif
 ...
 */
 ...
 {
  if ((Current == NULL) && (hw->html.formatted_elements != NULL))
  {
   FreeLineList(hw->html.formatted_elements);
   hw->html.formatted_elements = NULL;
  }
 ...
 */
 ...
 /* 
 * Addition by Xibios 2 from Axene for horizontak alignment manage 
 */
  
 ComputeAlignLineList(hw, hw->html.line_array, LineNumber, width);
...
#ifndef NTRACE
 gettimeofday(&Tv, &Tz);
 Xc_TRACE(("FormatAll exit (%d.%d)", Tv.tv_sec, Tv.tv_usec));
#endif
...
unsigned long fg, bg;
{
  ...
  if (height > 0)
  {
   if(!hw->html.bg_image)
    XFillRectangle(XtDisplay(hw), XtWindow(hw->html.view),
		   hw->html.drawGC, x, y, (unsigned int)all.width,
		   (unsigned int)height);
  }
 ...
 XFillRectangle(XtDisplay(hw), XtWindow(hw->html.view), hw->html.drawGC,
		x, y, width, ascent+eptr->font->descent);
       ...
       !hw->html.bg_image))
 {
  HTMLDrawBackgroundImage((Widget)hw,
			  ...
			  (y<0 ? 0 : y),
			  (x<0 ? (width+x) : width),
			  (y<0 ? (ascent+eptr->font->descent+y) :
			   (ascent+eptr->font->descent)));
 }

 if (eptr->font->per_char[160-eptr->font->min_char_or_byte2].attributes 
     == 3141)
 {
  char *duptdata;
  int i;

  duptdata = Xc_NormalStrDup("dt", tdata);
  for(i = 0; i < tlen; i++) 
   if (duptdata[i] == '\240') duptdata[i] = ' ';
  XDrawString(XtDisplay(hw), XtWindow(hw->html.view), hw->html.drawGC,
	      x, y + ascent, (char *)duptdata, tlen);
  free(duptdata);
 }
 else
  XDrawString(XtDisplay(hw), XtWindow(hw->html.view), hw->html.drawGC,
  ...
  if (width == -1)
  {
   ...
   if (eptr->font == hw->html.plain_font)
    ...
    XTextExtents(eptr->font, (char *)tdata,
		 tlen, &dir, &nascent, &descent,&all);
#else
   XTextExtents(eptr->font, (char *)tdata, tlen, &dir,
		&nascent, &descent,&all);
#endif /* ASSUME_FIXED_WIDTH_PRE */
   width = all.width;
  }
 ...
 if (eptr->strikeout == True)
 {
  ...
  if (width == -1)
  {
   ...
   if (eptr->font == hw->html.plain_font)
    ...
    XTextExtents(eptr->font, (char *)tdata, tlen, &dir, &nascent,
		 &descent,&all);
#else
   XTextExtents(eptr->font, (char *)tdata, tlen, &dir, &nascent,
		&descent,&all);
...
struct ele_rec *eptr;
{
 ...
 if (eptr->edata)		/* Ordered list bullet */
 {
 ...
 */
 ...
 if (eptr->indent_level < 2)
 {
  XFillArc(XtDisplay(hw), XtWindow(hw->html.view), hw->html.drawGC,
	   (x1 - width), y1, (width / 2), (width / 2), 0, 23040);
 }
 else
 {
  if (eptr->indent_level == 2)
  {
   XSetLineAttributes(XtDisplay(hw), hw->html.drawGC, 1,
		      LineSolid, CapButt, JoinBevel);
   XDrawRectangle(XtDisplay(hw), XtWindow(hw->html.view), hw->html.drawGC,
		  (x1 - width), y1, (width / 2), (width / 2));
  }
  else
  {
   if (eptr->indent_level > 2)
   {
    XSetLineAttributes(XtDisplay(hw), hw->html.drawGC, 1,
		       LineSolid, CapButt, JoinBevel);
    XDrawArc(XtDisplay(hw), XtWindow(hw->html.view), hw->html.drawGC,
	     (x1 - width), y1, (width / 2), (width / 2), 0, 23040);
   }
  }
 }
...
struct ele_rec *eptr;
{
 ...
 XDrawLine(XtDisplay(hw), XtWindow(hw->html.view),
	   hw->manager.bottom_shadow_GC, x1, y1, (int)(x1 + width), y1);
 XDrawLine(XtDisplay(hw), XtWindow(hw->html.view),
	   hw->manager.top_shadow_GC, x1, y1 + 1, (int)(x1 + width), y1 + 1);
}
...
struct ele_rec *eptr;
{
 ...
 if (eptr->pic_data != NULL)
 {
  ...
  if ((hw->html.border_images == True) ||
      ((eptr->anchorHRef != NULL) && (!eptr->pic_data->internal)))
  {
  ...
  if (extra)
  {
   ...
   XFillRectangle(XtDisplay(hw), XtWindow(hw->html.view), hw->html.drawGC,
		  ...
		  (eptr->pic_data->width + (2 * extra)), extra);
   XFillRectangle(XtDisplay(hw), XtWindow(hw->html.view), hw->html.drawGC,
		  x, y, extra, (eptr->pic_data->height + (2 * extra)));
   XFillRectangle(XtDisplay(hw), XtWindow(hw->html.view), hw->html.drawGC,
		  (x + eptr->pic_data->width + extra), y, extra,
   ...
   if (eptr->pic_data->image_data != NULL)
   {
    ...
    if (eptr->pic_data->transparent && eptr->pic_data->clip==None)
    {
     eptr->pic_data->clip = XCreatePixmapFromBitmapData
      (XtDisplay(hw), XtWindow(hw->html.view), eptr->pic_data->clip_data,
       eptr->pic_data->width, eptr->pic_data->height, 1, 0, 1);
    }
    else 
    {
      ...
      (!IsIsMapForm(hw, eptr->anchorHRef)))
  {
   XSetForeground(XtDisplay(hw), hw->html.drawGC, eptr->fg);
   XFillRectangle(XtDisplay(hw->html.view), XtWindow(hw->html.view),
		  hw->html.drawGC, x, (y + AnchoredHeight(hw)),
		  (eptr->pic_data->width + (2 * extra)), extra);
  }
 }
}
 ...
 while ((eptr != NULL) && (eptr->line_number <= (end + 1)))
 {
   ...
   */
   else
   {
    ...
    while (teptr != NULL)
    {
     if (teptr->type == E_LINEFEED) break;
     teptr = teptr->next;
    }
    if (teptr != NULL)
     ty2 = teptr->y + teptr->line_height - 1;
  ...
  if (eptr->type == E_TEXT)
  {
   ...
   tx1 = eptr->x;
   XTextExtents(eptr->font, (char *)eptr->edata,  eptr->edata_len - 1, &dir,
		&ascent, &descent, &all);
  ...
  {
   rptr = TableLocateElement(hw, eptr, x, y);
   break;
  }
 ...
 */
  ...
  */
  ...
  while (epos >= 0)
  {
   XTextExtents(rptr->font, (char *)rptr->edata,
		(epos + 1), &dir, &ascent, &descent, &all);
   if ((int)(rptr->x + all.width) <= x) break;
   epos--;
  }
  epos++;
  *pos = epos;
...
* to indent lines.
*/
char *ParseTextToString(elist, startp, endp, start_pos, end_pos,
			space_width, lmargin)
  ...
  case E_TABLE:
   {
    ...
    for(y = 0; y < eptr->table_data->numRows; y++)
    {
     for(x = 0; x < eptr->table_data->numColumns; x++)
     {
      field = &(eptr->table_data->table
		[y * eptr->table_data->numColumns + x]);
      if (field->first_rec && field->last_rec)
      {
       table_str = ParseTextToString(field->first_rec, field->first_rec,
				     field->last_rec->next, 0, 0,
				     space_width, lmargin);
       strcpy_or_grow(&text, &t_slen, &t_blen, table_str);
       free(table_str);
      }	
      strcpy_or_grow(&text, &t_slen, &t_blen, "\t");
     }
...
* to indent lines.
*/
char *ParseTextToPrettyString(hw, elist, startp, endp, start_pos, end_pos,
			      space_width, lmargin)
HTMLWidget hw;
struct ele_rec *elist;
struct ele_rec *startp;
 ...
 */
  ...
  */
  ...
  if (eptr->type == E_BULLET && !eptr->edata)
  {
   int i, spaces;
  ...
  else if (eptr->type == E_TEXT || (eptr->type == E_BULLET && eptr->edata))
  {
   int i, spaces;
   char *tptr;
   ...
   else if (eptr->type == E_TABLE)
   {
    ...
    for(y = 0; y < eptr->table_data->numRows; y++)
    {
     for(x = 0; x < eptr->table_data->numColumns; x++)
     {
      ...
      if (field->first_rec && field->last_rec)
      {
       table_str = ParseTextToPrettyString(field->first_rec,
					   field->first_rec,
					   field->last_rec->next, 0, 0,
					   space_width, lmargin);
       strcpy_or_grow(&text, &t_slen, &t_blen, table_str);
 ...
 */
  ...
  */
  ...
  {
   if ((listing_text) && (mptr->type == M_NONE))
   {
    ptr = mptr->text;
    while ((ptr != NULL) && (*ptr != '\0'))
    {
...
/* *********** Use in HTMLtable.c to manage Table BGCOLOR ************* */
typedef struct _tableInfo_stack_t
{
 unsigned long bg;
 XFontStruct	*currentFont;  
 int		lineNumber;
 ...
 int		Alignment;
 struct ele_rec *current;
 struct _tableInfo_stack_t *prev;
} tableInfo_stack_t;

static tableInfo_stack_t *tableInfo_stack_top = NULL;

void Push_hw_TableInfo()
{
 tableInfo_stack_t *tableInfo_stack;
  
 tableInfo_stack = (tableInfo_stack_t *)malloc(sizeof(tableInfo_stack_t));
  
 tableInfo_stack->bg = Bg;
 tableInfo_stack->width = Width;
 tableInfo_stack->maxWidth = MaxWidth;
 tableInfo_stack->currentFont = currentFont;
 tableInfo_stack->lineNumber = LineNumber;
 tableInfo_stack->lineHeight = LineHeight;
 tableInfo_stack->baseLine = BaseLine;
 tableInfo_stack->current = Current;
 tableInfo_stack->RAlignment = RAlignment;
 tableInfo_stack->Alignment = Alignment;
  
 tableInfo_stack->prev = tableInfo_stack_top;
 tableInfo_stack_top = tableInfo_stack;
  
 Width = 10000;
 Alignment = RAlignment = ALIGN_LEFT;
}

void Set_hw_TableInfo_bg(bg)
unsigned long bg;
{
 Bg = bg;
}

void Pop_hw_TableInfo()
{
 tableInfo_stack_t *tableInfo_stack;
  
 if (!tableInfo_stack_top) return;
  
 tableInfo_stack = tableInfo_stack_top;
  
 Bg = tableInfo_stack->bg;
 Width = tableInfo_stack->width;
 MaxWidth = tableInfo_stack->maxWidth;
 currentFont = tableInfo_stack->currentFont;
 LineNumber = tableInfo_stack->lineNumber;
 LineHeight = tableInfo_stack->lineHeight;
 BaseLine = tableInfo_stack->baseLine;
 Current = tableInfo_stack->current;
 RAlignment = tableInfo_stack->RAlignment;
 Alignment = tableInfo_stack->Alignment;
  
 tableInfo_stack_top = tableInfo_stack_top->prev;
 free(tableInfo_stack);
}
