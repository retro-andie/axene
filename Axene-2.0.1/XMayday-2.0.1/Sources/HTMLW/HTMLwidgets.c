...
caddr_t call_data;
{
 ...
 cbdata.attribute_count = CollectSubmitInfo
  (fptr, &cbdata.attribute_names, &cbdata.attribute_values);
 ...
 */
 while (XCheckWindowEvent(XtDisplay(hw->html.view), XtWindow(hw->html.view),
...
char *name;
{
 ...
 while (wptr != NULL)
 {
  if ((wptr->id >= fptr->start) && (wptr->type == W_RADIOBOX) &&
      (wptr->checked == True) && (wptr->name != NULL) && (name != NULL) &&
      (strcmp(wptr->name, name) == 0))
  {
   radio_checked = True;
   break;
...
WidgetInfo *AddNewWidget(hw, fptr, w, type, id, x, y, 
			 width, height, name, value, mapping, checked)
HTMLWidget hw;
FormInfo *fptr;
Widget w;
int type;
  ...
  *count = 0;
  ...
  while (*tptr != '\0')
  {
   if ((cnt + 1) == max_cnt)
   {
    ...
    if (tlist == NULL) return((char **)NULL);
   ...
   *tptr = '\0';
   ...
   if (list[cnt] == NULL) return((char **)NULL);
 ...
 */
  ...
  */
  ...
  else if ((type_str != NULL)&&(strcasecmp(type_str, "jot") == 0))
  {
   XmString label;
   Dimension width, height;
   Widget frame;
   ...
   */
   ...
   XtSetMappedWhenManaged(w, False);
   XtManageChild(w);
   if (label != NULL) XmStringFree(label);
  }
  ...
  else if ((type_str != NULL) && (strcasecmp(type_str, "password") ==0))
  {
   ...
   /*
   * The proper order here is XtSetMappedWhenManaged, XtManageChild. 
   * But a bug in some versions of Motif1.1 makes us do it the other way.
   * All versions of 1.2 should have this fixed
   */
#ifdef MOTIF1_2
   XtSetMappedWhenManaged(w, False);
   XtManageChild(w);
...
struct ele_rec *eptr;
{
 ...
 if ((eptr->widget_data != NULL) && (eptr->widget_data->w != NULL))
 {
  XtVaSetValues(eptr->widget_data->w, XmNx, eptr->x - hw->html.scroll_x,
		XmNy, eptr->y - hw->html.scroll_y, NULL);
  
  if (eptr->widget_data->mapped == False)
  {
 ...
 /* --- If we ever start using this, we need to set it up for bg image use. */
...
void traversal_forward(w, event, params, num_params)
Widget w;
XEvent *event;
String *params;
Cardinal *num_params;
{ 
 if (XtClass(XtParent(XtParent(w))) != htmlWidgetClass)
  return;

 HTMLTraverseTabGroups(XtParent(XtParent(w)), XmTRAVERSE_NEXT_TAB_GROUP);
}

void traversal_back(w, event, params, num_params)
Widget w;
XEvent *event;
String *params;
Cardinal *num_params;
{ 
 if (XtClass(XtParent(XtParent(w))) != htmlWidgetClass) return;

 HTMLTraverseTabGroups(XtParent(XtParent(w)), XmTRAVERSE_PREV_TAB_GROUP);
}

void traversal_end(w, event, params, num_params)
Widget w; 
XEvent *event;
String *params; 
Cardinal *num_params;
{
 ...
 HTMLTraverseTabGroups((Widget) hw, XmTRAVERSE_HOME);
  
 hw->html.forms_tabbing = False;
}

/* makes it possible to traverse forms */
void HTMLTraverseTabGroups(w, how)
Widget w;
int how;
{
 ...
 case XmTRAVERSE_NEXT_TAB_GROUP:
  ...
  if(!lptr->mapped)
   HTMLGotoId((Widget) hw, lptr->id);

  XtSetKeyboardFocus(w, lptr->w);
