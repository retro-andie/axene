...
extern int NoBodyColors ___PROTO((Widget w));
extern int NoBodyImages ___PROTO((Widget w));
extern int ParseMarkType();
extern int tableSupportEnabled;
extern void hw_do_bg();
extern void hw_do_color();
...
char *txt;
{
  ...
  */
   ...
   */
   ...
   if (*ptr == '\0')
   {
    Xc_TRACE(("warning:  unterminated & (%s)", start));
    unterminated = 1;
   }
   else
   {
   ...
   *ptr = '\0';
   ...
   if (text == NULL)
   {
    Xc_WARNING(("Cannot malloc space for & text"));
    *ptr = tchar;
    return;
   }
   strcpy(text, start);
...
char **endp;
{
 ...
 if (mark == NULL)
 {
  Xc_WARNING(("Cannot malloc space for mark_up struct"));
  return NULL;
 }
 ...
 */
 ...
 if (*ptr != '>')
 {
  Xc_TRACE(("error: bad mark format"));
  return NULL;
 }
 ...
 *ptr = '\0';
 ...
 if (text == NULL)
 {
  Xc_WARNING(("Cannot malloc space for mark"));
  *ptr = tchar;
  return NULL;
 }
 strcpy(text, start);
...
#ifndef NTRACE
 gettimeofday(&Tv, &Tz);
 Xc_TRACE(("HTMLParse enter (%d.%d)", Tv.tv_sec, Tv.tv_usec));
#endif
 ...
 */
  ...
  */
  if (text == NULL)
  {
   if (start != end)
   {
    Xc_WARNING(("error parsing text, bailing out"));
    return list;
   }
  }
  else
  {
   ...
   if (mark == NULL)
   {
    Xc_WARNING(("Cannot malloc for mark_up struct"));
    return list;
   }
   mark->type = M_NONE;
   mark->is_end = 0;
  ...
  */
  ...
  if (mark == NULL)
  {
   if (start != end)
   {
    Xc_WARNING(("error parsing mark, bailing out"));
    return list;
   }
  }
  else
  {
   ...
   if(mark->type == M_DOC_BODY && mark->start)
   {
    ...
    if (!NoBodyImages(hw))
    {
     tmp = ParseMarkTag(mark->start, MT_DOC_BODY, "background");
     if (tmp)
     {
      hw_do_bg(hw, tmp);
      free(tmp);
...
#ifndef NTRACE
 gettimeofday(&Tv, &Tz);
 Xc_TRACE(("HTMLParse exit (%d.%d)", Tv.tv_sec, Tv.tv_usec));
#endif
 ...
 *tptr = '\0';
 ...
 else if (caseless_equal(str, MT_LEFT))
 {
  type = M_LEFT;
 }
 ...
 else if (caseless_equal(str, MT_RIGHT))
 {
  type = M_RIGHT;
 }
 ...
 {
  Xc_TRACE(("warning: unknown mark (%s)", str));
  type = M_UNKNOWN;
 }
...
char **endp;
{
  ...
  */
   ...
   */
   ...
   if (tag_val == NULL)
   {
    Xc_WARNING(("can't malloc space for tag value"));
    return NULL;
   }
   strcpy(tag_val, "1");
  ...
  */
  if (quoted)
  {
   while ((*ptr != 34) && (*ptr != '\0'))/* double quote */
    ptr++;
  }
  else
  {
