...
METHODDEF void
read_backing_store (info, buffer_address, file_offset, byte_count)  backing_store_ptr info; void FAR * buffer_address;
...
METHODDEF void
write_backing_store (info, buffer_address, file_offset, byte_count)  backing_store_ptr info; void FAR * buffer_address;
		     long file_offset; long byte_count;
{
  if (fseek(info->temp_file, file_offset, SEEK_SET))
    ERREXIT(methods, "fseek failed on temporary file");
...
METHODDEF void
close_backing_store (info)  backing_store_ptr info;
{
