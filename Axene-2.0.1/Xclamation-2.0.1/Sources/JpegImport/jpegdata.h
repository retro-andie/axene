...
#endif
#define METHODN(type,methodname,arglist)  type (*methodname) ()
 ...
 */
	...
	UINT16 restarts_to_go;	/* MCUs left in this restart interval */
	short next_restart_num;	/* # of next RSTn marker (0..7) */
	char	*Output_Data;	/* pointer when decompress in memory */
	int	Output_Pos;	/* position depuis le debut du buffer */
};
...
struct External_methods_struct {
	...
	METHODN(JSAMPARRAY, access_big_sarray, (big_sarray_ptr ptr,
						long start_row,
						boolean writable));
	METHODN(JBLOCKARRAY, access_big_barray, (big_barray_ptr ptr,
						 long start_row,
						 boolean writable));
