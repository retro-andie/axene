 ...
 */
png_voidp
png_create_struct(type)  uInt type;
{
   png_size_t type;
...
void
png_destroy_struct(struct_ptr)  png_voidp struct_ptr;
{
   if (struct_ptr)
      farfree (struct_ptr);
}
...
png_voidp
png_large_malloc(png_ptr, size)  png_structp png_ptr; png_uint_32 size;
{
   png_voidp ret;
   if (!png_ptr || !size)
      return ((voidp)NULL);
...
void
png_large_free(png_ptr, ptr)  png_structp png_ptr; png_voidp ptr;
{
   if (!png_ptr)
      return;
...
void *
png_malloc(png_ptr, size)  png_structp png_ptr; png_uint_32 size;
{
   void *ret;
...
void *
png_realloc(png_ptr, ptr, size, old_size)  png_structp png_ptr; void * ptr; png_uint_32 size;
...
/* free a pointer allocated by png_malloc().  In the default
  ...
  is needed.  If ptr is NULL, return without taking any action. */
void
png_free(png_ptr, ptr)  png_structp png_ptr; void * ptr;
{
   if (!png_ptr)
      return;
