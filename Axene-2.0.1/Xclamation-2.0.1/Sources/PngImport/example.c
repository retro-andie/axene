...
int
initialize_png_reader(png_ptr, info_ptr)  png_structp *png_ptr; png_infop *info_ptr;
{
   /* Create and initialize the png_struct with the desired error handler
      functions.  If you want to use the default stderr and longjump method,
      you can supply NULL for the last three parameters.  We also check that
...
int
process_data(png_ptr, info_ptr, buffer, length)  png_structp *png_ptr; png_infop *info_ptr;
...
info_callback(png_ptr, info)  png_structp png_ptr; png_infop info;
{
...
row_callback(png_ptr, new_row, row_num, pass)  png_structp png_ptr; png_bytep new_row;
...
end_callback(png_ptr, info)  png_structp png_ptr; png_infop info;
{
