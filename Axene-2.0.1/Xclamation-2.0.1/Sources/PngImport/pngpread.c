...
void
png_process_data(png_ptr, info, buffer, buffer_size)
png_structp png_ptr;
png_infop info;
png_bytep buffer;
...
void
png_process_some_data(png_ptr, info)  png_structp png_ptr; png_infop info;
{
...
void
png_push_read_sig(png_ptr)  png_structp png_ptr;
{
 png_byte sig[8];
...
void
png_push_read_chunk(png_ptr, info)  png_structp png_ptr; png_infop info;
{
...
void
png_push_check_crc(png_ptr)  png_structp png_ptr;
{
 png_byte crc_buf[4];
 png_uint_32 crc;
...
void
png_push_crc_skip(png_ptr, length)  png_structp png_ptr; png_uint_32 length;
{
...
void
png_push_skip(png_ptr)  png_structp png_ptr;
{
...
void
png_push_fill_buffer(png_ptr, buffer, length)  png_structp png_ptr; png_bytep buffer;
png_uint_32 length;
{
 png_bytep ptr;
...
void
png_push_save_buffer(png_ptr)  png_structp png_ptr;
{
 if (png_ptr->save_buffer_size)
 {
  if (png_ptr->save_buffer_ptr != png_ptr->save_buffer)
  {
   ...
   for (i = 0, sp = png_ptr->save_buffer_ptr, dp = png_ptr->save_buffer;
	i < (int)png_ptr->save_buffer_size;
	i++, sp++, dp++)
   {
    *dp = *sp;
   }
...
void
png_push_restore_buffer(png_ptr, buffer, buffer_length)  png_structp png_ptr; png_bytep buffer;
...
void
png_push_read_IDAT(png_ptr)  png_structp png_ptr;
{
 if (!(png_ptr->flags & PNG_FLAG_HAVE_CHUNK_HEADER))
 {
  png_byte chunk_start[8];
...
void
png_process_IDAT_data(png_ptr, buffer, buffer_length)  png_structp png_ptr; png_bytep buffer;
png_uint_32 buffer_length;
{
 int ret;
...
void
png_push_process_row(png_ptr)  png_structp png_ptr;
{
...
void
png_read_push_finish_row(png_ptr)  png_structp png_ptr;
{
 png_ptr->row_number++;
...
void
png_push_handle_PLTE(png_ptr, length)  png_structp png_ptr; png_uint_32 length;
{
 if (length % 3)
...
void
png_push_read_PLTE(png_ptr, info)  png_structp png_ptr; png_infop info;
{
...
#if defined(PNG_READ_tEXt_SUPPORTED)
void
png_push_handle_tEXt(png_ptr, length)  png_structp png_ptr; png_uint_32 length;
{
...
void
png_push_read_tEXt(png_ptr, info)  png_structp png_ptr; png_infop info;
{
...
#if defined(PNG_READ_zTXt_SUPPORTED)
void
png_push_handle_zTXt(png_ptr, length)  png_structp png_ptr;
png_uint_32 length;
{
 png_ptr->current_text = (png_charp)png_large_malloc(png_ptr, length + 1);
 png_ptr->current_text[(png_size_t)length] = '\0';
...
void
png_push_read_zTXt(png_ptr, info)  png_structp png_ptr; png_infop info;
{
 if (png_ptr->buffer_size && png_ptr->current_text_left)
 {
  png_uint_32 text_size;
...
void
png_push_have_info(png_ptr, info)  png_structp png_ptr; png_infop info;
{
 if (png_ptr->info_fn)
...
void
png_push_have_end(png_ptr, info)  png_structp png_ptr; png_infop info;
{
 if (png_ptr->end_fn)
...
void
png_push_have_row(png_ptr, row)  png_structp png_ptr; png_bytep row;
{
 if (png_ptr->row_fn)
...
png_voidp
png_get_progressive_ptr(png_ptr)  png_structp png_ptr;
{
 return png_ptr->io_ptr;
}

void
png_push_read_end(png_ptr, info)  png_structp png_ptr; png_infop info;
{
 if (!(png_ptr->flags & PNG_FLAG_HAVE_CHUNK_HEADER))
 {
  png_byte chunk_start[8];
...
void
png_set_progressive_read_fn(png_ptr, progressive_ptr, info_fn, row_fn, end_fn)  png_structp png_ptr; png_voidp progressive_ptr;
...
void
png_progressive_combine_row (png_ptr, old_row, new_row)  png_structp png_ptr;
