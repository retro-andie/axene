#ifndef lint
static char rcsid[] = "$Header: /usr/people/sam/tiff/libtiff/RCS/tif_print.c,v 1.49 93/08/26 14:25:40 sam Exp $";
#endif
...
#ifdef JPEG_SUPPORT
static void JPEGPrintQTable(fd, tab)
FILE* fd;
u_char* tab;
{
 int i, j;
...
static void JPEGPrintCTable(fd, tab)
FILE* fd;
u_char* tab;
{
 int i, n, count;
...
long flags;
{
 ...
 if (TIFFFieldSet(tif,FIELD_MINSAMPLEVALUE))
  fprintf(fd, "  Min Sample Value: %u\n", 
	  (unsigned int)td->td_minsamplevalue);
 if (TIFFFieldSet(tif,FIELD_MAXSAMPLEVALUE))
  fprintf(fd, "  Max Sample Value: %u\n", 
	  (unsigned int)td->td_maxsamplevalue);
