#ifndef lint
static char rcsid[] = "$Header: /usr/people/sam/tiff/libtiff/RCS/tif_getimage.c,v 1.20 93/08/25 09:28:15 sam Exp $";
#endif
...
static	int gt ___PROTO((TIFF*, int, int, u_long*));
static	int gtTileContig ___PROTO((TIFF*, u_long*, RGBvalue*, u_long, u_long));
static	int gtTileSeparate ___PROTO((TIFF*, u_long*, RGBvalue*, u_long, u_long));
static	int gtStripContig ___PROTO((TIFF*, u_long*, RGBvalue*, u_long, u_long));
static	int gtStripSeparate ___PROTO((TIFF*, u_long*, RGBvalue*, u_long, u_long));
static	int makebwmap ___PROTO((TIFF*, RGBvalue*));
static	int makecmap ___PROTO((TIFF*, u_short*, u_short*, u_short*));
static	void initYCbCrConversion ___PROTO((void));
 ...
 */ 
static int
setupMap(tif, minsamplevalue, maxsamplevalue, pMap)
TIFF* tif;
...
typedef void (*tileContigRoutine) ___PROTO((u_long*, u_char*, RGBvalue*, u_long, u_long, int, int));
static tileContigRoutine pickTileContigCase ___PROTO((TIFF*, RGBvalue*));
...
typedef void (*tileSeparateRoutine) ___PROTO((u_long*, u_char*, u_char*, u_char*, RGBvalue*, u_long, u_long, int, int));
static tileSeparateRoutine pickTileSeparateCase ___PROTO((TIFF*, RGBvalue*));
...
static void name(cp, pp, Map, w, h, fromskew, toskew)\
     u_long* cp; \
     u_char* pp; \
     RGBvalue* Map; \
     u_long w; \
...
DECLAREContigPutFunc(putRGBcontig8bittile)
{
  ...
  else
  {
    while (h-- > 0)
    {
      UNROLL8(w, x = x, *cp++ = PACK(pp[0], pp[1], pp[2]);
	      pp += samplesperpixel);
      cp += toskew;
      pp += fromskew;
    }
...
static void name(cp, r, g, b, Map, w, h, fromskew, toskew)\
     u_long* cp; \
     u_char* r; \
     u_char* g; \
     u_char* b; \
