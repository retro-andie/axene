...
char *ch;
{
  ...
  if(gcmpgs(nlig,MAXHALFULONG)>=0)
    err(talker,"too many rows in matrix");
  if(gcmpgs(ncol,MAXHALFULONG)>=0)
...
void ps_point ___PROTO((FILE *psfile, int x, int y)),ps_line ___PROTO((FILE *psfile, int x1, int y1, int x2, int y2)),ps_rect ___PROTO((FILE *psfile, int x1, int y1, int x2, int y2)),ps_string ___PROTO((FILE *psfile, int x, int y, char *c));
