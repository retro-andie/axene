...
GEN
cleanmod(x, lim, detmat, detmatsur2)
GEN x;long lim;GEN detmat;GEN detmatsur2;
{
  long lx=lg(x),i;
  GEN y,p1;
...
GEN
allhnfmod(x, detmat, all)
GEN x;GEN detmat;long all;
{
...
long prec;
{
  GEN y,p1,p2,p3,p4,p5,p6,p7,p10,p11,p12,fieldd,polr,ptrace,dx,adx,polmax,s,a,dxn,adxn,sn,phimax,ind,fa;
  ...
  p2=gcmp0(p2)?p4:lift(gsubst(p4,v,polymodrecip(p2)));
  p3=cgetg(n+1,19);
  for(j=1;j<=n;j++)
  {
...
/* a usage interne de galoisconj */
{
  ...
  avma=av;e=(e1+n+1)/ep+10; /* a modifier */
  return e;
}
