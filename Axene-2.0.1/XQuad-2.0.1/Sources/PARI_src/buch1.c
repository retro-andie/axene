...
GEN gCO;
{
  ...
  while(s<mm)
  {
    ...
    nbtest++;
#ifdef DEBUG_MODE
    if((DEBUGLEVEL>=2)&&(!fpc)) fprintferr(".");
#endif
    if(fpc>1)
    {
  ...
  while(s<kc2)
  {
    ...
    pp=base[s+1];
#ifdef DEBUG_MODE
    if(DEBUGLEVEL>=2) fprintferr(" %ld",pp);
#endif
...
long *
subfactorbaseimag(d, w, ll, kc, vectprime, vperm, ptnbram)
GEN d;
GEN *w;
double ll;
long kc;
...
long
factorbasequad(d, n2, n, ptnum, ptbase, ptkc, badprim, nbbadprim)
GEN d;
long n2;
long n;
long **ptnum;
...
long
factorisequad(f, n, limp, ptlo, primfact, expoprimfact, badprim, nbbadprim, base, limhash)
GEN f;
long n;
long limp;
long *ptlo;
...
long *
largeprime2(q1, ex, np, cardsubbase, hashtab)
long q1;
long *ex;
long np;
long cardsubbase;
...
GEN
buchreal(D, gsens, gcbach, gcbach2, gRELSUP, prec)
GEN D;
GEN gsens;
GEN gcbach;
GEN gcbach2;
...
long *
subfactorbasereal(d, w, ll, precreg, kc, vectprime, vperm, ptnbram, isqrtD, sqrtD, sens)
GEN d;
GEN *w;
double ll;
long precreg;
...
GEN **
powsubfactreal(w, n, a, D, isqrtD, sqrtD, sens, precreg)
GEN w;
long n;
long a;
GEN D;
...
long *
largeprime(q1, ex, np, nrho, cardsubbase, hashtab)
long q1;
long *ex;
long np;
long nrho;
...
GEN
comprealform3(x, y, D, isqrtD, sens)
GEN x;
GEN y;
GEN D;
GEN isqrtD;
...
GEN
comprealform5(x, y, D, isqrtD, sqrtD, sens)
GEN x;
GEN y;
GEN D;
GEN isqrtD;
...
GEN
sqrealform5(x, D, isqrtD, sqrtD, sens)
GEN x;
GEN D;
GEN isqrtD;
GEN sqrtD;
...
GEN
redrealform5(x, D, isqrtD, sqrtD, sens)
GEN x;
GEN D;
GEN isqrtD;
GEN sqrtD;
...
GEN
redrealform(x, D, isqrtD, sqrtD, sens, precreg)
GEN x;
GEN D;
GEN isqrtD;
GEN sqrtD;
...
GEN
powrealform5(x, n, D, isqrtD, sqrtD, sens, precreg)
GEN x;
long n;
GEN D;
GEN isqrtD;
...
GEN
initializeform3(ex, tabform, cardtab, d, isqrtd, sens)
long *ex;
GEN **tabform;
long cardtab;
GEN d;
...
GEN
initializeform5(ex, tabform, cardtab, d, isqrtd, sqrtd, sens)
long *ex;
GEN **tabform;
long cardtab;
GEN d;
...
long prec;
{
  ...
  *precreg=max(prec+1,(gexpo(D)>>TWOPOTBITS_IN_LONG)+5);
