...
extern int T1Close(), T1Getc(), T1Ungetc(), T1Read();
 
#define  _fclose(f)          T1Close(f)
#define  _fopen(name,mode)   T1Open(name,mode)
#define  _ungetc(c,f)        T1Ungetc(c,f)
#define  _fgetc(f)           T1Getc(f)
#define  _fread(bufP,size,n,f) T1Read(bufP,size,n,f)
#define  _feof(f)            (((f)->flags & FIOEOF) && ((f)->b_cnt==0))
#define  _ferror(f)          (((f)->flags & FIOERROR)?(f)->error:0)
