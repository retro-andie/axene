/*
** Spread_Verticaly_Region.c for XQuad in Feuille/
** Spread region verticaly
**
...
** Last update Fri Dec 13 18:38:23 1996 Emmanuel Paris
*/

#include "Select_Move_Resize_Cells.h"

void spread_verticaly_region_init();
void spread_verticaly_region_abort();
void spread_verticaly_region_done();
void spread_verticaly_region_to_xy();
void spread_verticaly_region_trace();

void spread_verticaly_region_init(cfeuille,xm,ym)
c_Feuille *cfeuille;
int	  xm;
int	  ym;
{
...
void spread_verticaly_region_abort(cfeuille)
c_Feuille *cfeuille;
{ 
  DATA(Select_Move_Resize_Cell);
  ...
  spread_verticaly_region_trace(cfeuille, TRUE);
  d(source)=FALSE;  d(source) = FALSE;
}

void spread_verticaly_region_done(cfeuille)
c_Feuille *cfeuille;
{  
  region_t region;
  DATA(Select_Move_Resize_Cell);
  ...
  if (cfeuille->cell_region->y_min == d(region).y_min)
  ...
  else
    {
      ...
      region.y_max = cfeuille->cell_region->y_min - 1;
    }
...
void spread_verticaly_region_to_xy(cfeuille,xm,ym)
c_Feuille *cfeuille;
int	xm;
int	ym;
{ 
  ...
  if (F(cfeuille).GetRowByY(cfeuille, ym, &row) )
	...
	if (row < cfeuille->cell_region->y_min)
	  ...
	  F(CURSOR).set_cursor(CURSOR, C_SPREAD_V_CENTER_REGION);
...
void spread_verticaly_region_trace(cfeuille)
c_Feuille *cfeuille;
{  
  Display	*display;
  Window		window;
