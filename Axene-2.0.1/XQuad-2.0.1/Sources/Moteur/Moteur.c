...
** Last update Thu Jan  6 23:36:40 2000 Emmanuel Paris
*/
...
static void Moteur_simple_set_string();
...
/* ----------------------------------------------------------------- ** 
** remove_depend - store depend in list (internal)                   ** 
** ----------------------------------------------------------------- */
void remove_depend(col, line, all_dep)
int		col;
int		line;
cell_depend	**all_dep;
{
 ...
 dep = all_dep;
 while (*dep && (((*dep)->line != line) || ((*dep)->column != col)) )
 {
  dep = &((*dep)->Next);
 }
 if (*dep)
...
** remove_depend - store depend in list (internal)                   ** 
** ----------------------------------------------------------------- */
void remove_depend2(cell_s, all_dep)
cell		*cell_s;
cell_depend	**all_dep;
{
 cell_depend	**dep;
...
int		type;
{
    ...
    case Xq_EXPONANTIAL:
     {
      ...
      tmp_str = g2estr(cell_s->value2, LOW_PREC);
      i = 0;
      while(tmp_str[i] && tmp_str[i] != 'E')
      {
       str[i] = tmp_str[i];
       i++;
      }
 ...
 case Xq_VALUE:
  ...
  if (cell_s->value2)
  {
   ...
   if ((int)cell_s->ret_type == TYPE_EXPONANTIAL)
    tmp_str = g2estr(cell_s->value2, LOW_PREC);
   else
   {
    tmp_str = g2str(cell_s->value2, LOW_PREC);
    rlen = strlen(tmp_str);
    slen = rlen-1;
    while (slen && (tmp_str[slen]!='E'))
     slen --;
    if (tmp_str[slen]=='E')
    {
     cell_s->ret_type = TYPE_EXPONANTIAL;
    }
    else
    {
     slen = 0;
...
HG_list		**all_gdep;
{
 ...
 cell_depend_list	**list_tmp2 = NULL;
 ...
 while (list_tmp1)
 {
  ...
  if (list_tmp1->depend.cell_p->formula)
  {
...
c_Feuille	*sel_feuille;
{
 ...
 if (cell_s->value2)
 {
  if(cell_s->ret_type == TYPE_EXPONANTIAL)
   str_val = g2estr(cell_s->value2, BIG_PREC);
  else
 ...
 if ((str[i]=='+') || (str[i]=='-') || (str[i]>='0' && str[i]<='9'))
 {
  ...
  if ((str[i]>='0' && str[i]<='9'))
  {
   i++;
   prec++;
   while ((str[i] >= '0') && (str[i] <= '9') && (prec < LOW_PREC))
   {
    i++;
    prec++;
   }
   ...
   if (str[i] == 'e' || str[i] == 'E')
   {
    ...
    while (str[i] == ' ')
     i++;
    max = 0;
    while((str[i] >= '0' && str[i] <= '9') && (max < 6))
    {
     exp_real = (exp_real*10)+str[i]-'0';
     max++;
     i++;
    }
    exp_real *= exp_sign;
    ...
    strncpy(fin_str, str + i_space, prec_pos - i_space);
    fin_str[prec_pos - i_space] = 0;
    sprintf(strtmp, "E%d", exp_real);
    strcat(fin_str, strtmp);
    cell_s->ret_type = TYPE_EXPONANTIAL; 
   }
   else if (prec_pos)
   {
...
/* ------------------------------------------------------------------- ** 
** simple_set_string - set a simple string in cell (called by EditBar) ** 
** ------------------------------------------------------------------- */
static void Moteur_simple_set_string(This, x, y, cell_s, str, sel_feuille)
c_Moteur	*This;
int		x;
int		y;
cell 		**cell_s;
...
c_Feuille	*sel_feuille;
{
 ...
 (*cell_s)->text = (char *)Xc_strdup("set_str_txt", str);
...
/* ----------------------------------------------------------------- ** 
** store_checked - create tree for check result                      ** 
** ----------------------------------------------------------------- */
static int store_checked(cell_d, list_tmp, root, depth, cell_cmp)
cell_depend		*cell_d;
cell_depend_list	**list_tmp;
cell_depend_list	**root;
int			depth;
  ...
  if (!branch->expanded)
  {
   if (branch->depend.cell_p->depend)
   {
    Xc_TRACE(("Branch %c%d",
	      branch->depend.column+'a',
	      branch->depend.line+1));
    if (store_checked(branch->depend.cell_p->depend,
		      &(branch->depend.Next), 
		      root, depth+1, cell_cmp) == -1)
     return -1;
   }
   branch->expanded = TRUE;
  }
...
/* ----------------------------------------------------------------- ** 
** free_stored_check - free tree for check result                    ** 
** ----------------------------------------------------------------- */
static void free_stored_check(list)
cell_depend_list	*list;
{
 cell_depend_list	*list_tmp;
...
int		y;
{
 ...
 list_d = NULL;
 depth = 0;
 if (store_checked(cell_s->depend, &list_d, &list_d, depth, tmp_cell) == -1)
 {
  free_stored_check(list_d);
  return TRUE;
 }
 else
 {
...
cell 		*cell_d;
{
 ...
 list_d = NULL;
 depth = 0;
 if (store_checked(cell_d->depend, &list_d, &list_d, depth, cell_s) == -1)
 {
  free_stored_check(list_d);
  return TRUE;
 }
...
** ----------------------------------------------------------------- */
static int frag_style(This, region, cstyle)
c_Moteur	*This;
region_t	*region;
CellStyle_t	*cstyle;
{
 ...
 while (*list && ((*list)->region.x_min <= region->x_max))
 {
  if (region->x_min > (*list)->region.x_min)
  {
   if (region->x_min <= (*list)->region.x_max)
   {
	...
	/* cas 1.1 */
	if ((region->y_max == (*list)->region.y_max)
	    && (region->x_max == (*list)->region.x_max)
	    && ((*list)->cstyle == cstyle))
	{
	 (*list)->id = This->id_func;
	 return 0;
	}
	...
	(*list)->region.y_max = region->y_min-1;
     ...
     else
     {
      if (region->y_max >= (*list)->region.y_max)
      {
       /* cas 1.3*/
       if ((region->y_min == (*list)->region.y_min)
	   && (region->y_max == (*list)->region.y_max)
	   && (region->x_max == (*list)->region.x_max)
	   && ((*list)->cstyle == cstyle))
       {
	(*list)->id = This->id_func;
	...
	(*list)->region.y_max = region->y_min-1;
     ...
     else
     {
      if (region->y_max >= (*list)->region.y_max)
      {
       /* cas 2.3*/
       if ((region->y_max == (*list)->region.y_max) 
	   && (region->y_min == (*list)->region.y_min) 
	   && ((*list)->cstyle == cstyle))
       {
	(*list)->id = This->id_func;
	return 0;
    ...
    if (region->y_min > (*list)->region.y_min)
    {
     if (region->y_min <= (*list)->region.y_max)
     {
      if (region->y_max >= (*list)->region.y_max)
      {
       /* cas 3.1 */
       if ((region->y_max == (*list)->region.y_max)
	   && (region->x_min == (*list)->region.x_min)
	   && (region->x_max == (*list)->region.x_max)
	   && ((*list)->cstyle == cstyle))
       {
	(*list)->id = This->id_func;
...
c_Moteur	*This;
{
 ...
 while (list_tmp)
 {
  ...
  while (tmp_gato)
  {
   if(tmp_gato->ptr == list_tmp->cstyle)
    break;
   tmp_gato = tmp_gato->Next;
  }
  if (tmp_gato == NULL)
  {
   *cur_gato = (t_gato *)Xc_malloc("gato", sizeof(t_gato));
   (*cur_gato)->ptr = (void *)list_tmp->cstyle;
   (*cur_gato)->count = list_tmp->cstyle->count;
   (*cur_gato)->act_count = 1;
   (*cur_gato)->Next = NULL;
   cur_gato = &((*cur_gato)->Next);
  }
  else
   tmp_gato->act_count++;
      
  printf("%p (%ld)- xmin: %d, xmax:%d, ymin:%d, ymax:%d\n", 
	 list_tmp->cstyle,
	 list_tmp->cstyle->count,
 ...
 while (tmp_gato)
 {
  if((tmp_gato->count != tmp_gato->act_count)
     && (tmp_gato->ptr != This->BaseStd->cell_style_base->default_cstyle))
   printf("BEEP BEEP %p (%d) is %d\n", 
	  tmp_gato->ptr,
	  tmp_gato->count,
	  tmp_gato->act_count);
  tmp_gato = tmp_gato->Next;
 ...
 printf("*Base\n");
 init_style = This->BaseStd->cell_style_base->cell_style;
 while (init_style)
 {
  printf("%p (%ld)\n", init_style, init_style->count);
  init_style = init_style->NextCellStyle;
 }
}
#endif
...
region_t	*region;
{
 ...
 if (frag_style(This, region, cstyle))
 {
  XqCS_INC_REFERENCE(cstyle);
...
int		y;
{
 ...
 while (list_start)
 {
  ...
  XqCS_DEC_REFERENCE(list_start->cstyle);
 ...
 while (src[sindex]!=0)
 {
  while (((src[sindex] < 'a') ||
	 ...
	 ((src[sindex] < 'A') ||
	  (src[sindex] > 'Z')) &&
	 (src[sindex] != '$') &&
	 (src[sindex] != 34) && /* double quote */
	 (src[sindex] != 0))
  {
   buffer[dindex++] = src[sindex++];
  }
      
  if (src[sindex] == 34)	/* double quote */
  {
   buffer[dindex++] = src[sindex++];
   while ((src[sindex] != 34) &&/* double quote */
	  (src[sindex] != 0))
    buffer[dindex++] = src[sindex++];
   if (src[sindex] == 34)	/* double quote */
   {
...
int generate_cutbuffer(This, region, fstr)
c_Moteur	*This;
region_t	*region;
char		**fstr;
{
...
#define Xq_FORMULA_LENGTH2 5000

 char	str[Xq_FORMULA_LENGTH2];

 *fstr = Xc_malloc("cutbuffer", Xq_FORMULA_LENGTH2);
 max_size = Xq_FORMULA_LENGTH2;
 size = 0;
 ry = region->y_min;
 while (ry <= region->y_max)
 {
  ...
  {
   tmp_cell = F(This).get_cell(This, rx, ry);
   rx++;
   if (tmp_cell != &voidcell)
   {
    Moteur_get_string(This, tmp_cell, str, Xq_VALUE);
    tmp_size= strlen(str);
    if (size+tmp_size > max_size)
    {
     max_size += Xq_FORMULA_LENGTH2;
     if (max_size > 32000)
     {
     ...
     fstr_tmp = Xc_malloc("cutbuffer", max_size);
     if (!fstr_tmp)
     {
      return size;
     }
     strcpy(fstr_tmp, *fstr);
     Xc_free(*fstr);
     *fstr = fstr_tmp;
    }
    strcpy(*fstr+size, str);
    size += tmp_size;
   }
   if (size+1 >= max_size)
   {
...
int		mode;
{
 ...
 This->mode = mode;
 /* generate big string */
 size = generate_cutbuffer(This, region, &cutbuf);
 /* should unselect the editbar */
 XmTextFieldClearSelection(WIDGET(GlobEditBar->Edit), CurrentTime);
 XRotateBuffers(XtDisplay(WIDGET(GlobEditBar)), 1);
 XStoreBytes(XtDisplay(WIDGET(GlobEditBar)), cutbuf, strlen(cutbuf));
 Xc_free(cutbuf);
}
...
** ----------------------------------------------------------------- */
static region_t *Moteur_paste_zone(This, dstRegion, x, y)
c_Moteur	*This;
region_t	*dstRegion;
int		x;
int		y;
{
 region_t	*region;
 region_t	**ptrrdw_region;
 region_t	*redraw_region;
 region_t	*tregion;
 region_t	rregion;
 ...
 if (This->mode == Xc_MODE_CUT)
 {
  ...
  redraw_region = Xc_malloc("region pasted", sizeof(region_t));
  ...
  redraw_region->x_min = x;
  redraw_region->y_min = y;
  redraw_region->x_max = x+(rregion.x_max-rregion.x_min);
  redraw_region->y_max = y+(rregion.y_max-rregion.y_min);

  Xc_TRACE(("cut-paste"));
  redraw_region->NextRegion = Xc_malloc("region cuted", sizeof(region_t));
  redraw_region->NextRegion->x_min = region->x_min;
  redraw_region->NextRegion->y_min = region->y_min;
  redraw_region->NextRegion->x_max = region->x_max;
  redraw_region->NextRegion->y_max = region->y_max;
  redraw_region->NextRegion->NextRegion = NULL;
 ...
 else
 {
  ...
  tregion = dstRegion;
  ptrrdw_region = &redraw_region;
  while (tregion)
  {
   /*--- case of same origin and not a bigger region ---*/
   if ((tregion->x_min == region->x_min) && (tregion->y_min == region->y_min)
       && (tregion->x_max < region->x_max) && (tregion->x_max < region->x_max))
    continue;

   /*--- cas d'une cellule simple ---*/
   if ((tregion->x_min == tregion->x_max) && 
       (tregion->y_min == tregion->y_max))
   {
    /*--- clip le depassement de la zone autorise ---*/
    rregion.x_min = region->x_min;
    rregion.y_min = region->y_min;
    rregion.x_max = tregion->x_min+(region->x_max-region->x_min);
    if (rregion.x_max > COLUMN_MAX)
     rregion.x_max = region->x_max-(rregion.x_max-COLUMN_MAX);
    else
     rregion.x_max = region->x_max;

    rregion.y_max = tregion->y_min+(region->y_max-region->y_min);
    if (rregion.y_max > ROW_MAX)
     rregion.y_max = region->y_max-(rregion.y_max-COLUMN_MAX);
    else
     rregion.y_max = region->y_max;

    *ptrrdw_region = Xc_malloc("region pasted", sizeof(region_t));
    (*ptrrdw_region)->x_min = tregion->x_min;
    (*ptrrdw_region)->y_min = tregion->y_min;
    (*ptrrdw_region)->x_max = tregion->x_min+(rregion.x_max-rregion.x_min);
    (*ptrrdw_region)->y_max = tregion->y_min+(rregion.y_max-rregion.y_min);
    (*ptrrdw_region)->NextRegion = NULL;
    ptrrdw_region = &((*ptrrdw_region)->NextRegion);

    F(This).copy_style(This, &rregion, tregion->x_min, tregion->y_min);
    F(This).copy_zone(This, &rregion, tregion->x_min, tregion->y_min);
   }
   else
   {
    int	cur_x;
    ...
    *ptrrdw_region = Xc_malloc("region pasted", sizeof(region_t));
    (*ptrrdw_region)->x_min = tregion->x_min;
    (*ptrrdw_region)->y_min = tregion->y_min;
    (*ptrrdw_region)->x_max = tregion->x_max;
    (*ptrrdw_region)->y_max = tregion->y_max;
    (*ptrrdw_region)->NextRegion = NULL;
    ptrrdw_region = &((*ptrrdw_region)->NextRegion);
    ...
    rregion.x_max = region->x_max;
    cur_x = tregion->x_min;
    while ((tregion->x_max - cur_x) > (region->x_max - region->x_min))
    {
     rregion.y_max = region->y_max;
     cur_y = tregion->y_min;
     while ((tregion->y_max - cur_y) > (region->y_max - region->y_min))
     {
      F(This).copy_style(This, &rregion, cur_x, cur_y);
      F(This).copy_zone(This, &rregion, cur_x, cur_y);
      cur_y += (region->y_max - region->y_min)+1;
     }
     rregion.y_max = region->y_min+(tregion->y_max - cur_y);
     F(This).copy_style(This, &rregion, cur_x, cur_y);
     F(This).copy_zone(This, &rregion, cur_x, cur_y);
     cur_x += (region->x_max - region->x_min)+1;
    }
    rregion.x_max = region->x_min+(tregion->x_max - cur_x);
    rregion.y_max = region->y_max;
    cur_y = tregion->y_min;
    while ((tregion->y_max - cur_y) > (region->y_max - region->y_min))
    {
  ...
  while (tmpcol && (tmpcol->number <= region->x_max))
  {
   ...
   while (numline != test)
   {
	...
	!lastwasval)
    {
     int	i;
     c_line *tmplinetmp;
     c_line *tmplinelast = NULL;

     real_d = 0.0;
     real_m = 0.0;
     tmplinetmp = tmplinezone;
     real_s = (real)gtodouble2(tmplinetmp->cell.value2);
     while (tmplinetmp)
     {
	...
	break;
     ...
     else
     {
      ...
      real_e = (real)gtodouble2(tmplinelast->cell.value2);
      genlast = tmplinelast->cell.value2;
	...
	test =(real)gtodouble2(tmplinelast->cell.value2);
...
HG_list		**gdep;
{
 ...
 /* sub dep already done in del_keep_cell */
 Moteur_delkeep_cell(This, x_src, y_src, cellp, dep, gdep);

 /*remove_depend(x_src, y_src, dep);*/
 /*remove_depend2(cells, dep);*/
