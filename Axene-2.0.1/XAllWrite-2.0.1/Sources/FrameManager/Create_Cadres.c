/*
** Create_Cadres.c for XAllWrite in FrameManager/
...
** Last update Tue Jul  7 18:39:26 1998 Emmanuel Paris
*/
...
c_FrMg	*cfrmg;
{
 ...
 else
 {
  ...
  cfrmg->modify_cadre_callback(cfrmg->callback_data, 
			       ((cadre->textflow_mode & XcTFM_HOLE_MASK) > 
				XcTFM_OVER));
...
void init_Create_Cadre_Elipse(cfrmg)
c_FrMg	*cfrmg;
{
 Window		root;
 Window		fille;
...
status_t eventlook_Create_Cadre_Elipse(cfrmg, event)
c_FrMg	*cfrmg;
XEvent	event;
{
 char		tamp_cla[20];
 ...
 case ButtonRelease:
  if (d(source) && event.xbutton.time > d(time) + 600)
  {
   Create_Cadre_Elipse_Done(cfrmg);
   F(CURSOR).pop_cursor(CURSOR);
   FREE_DATA;
...
void abort_Create_Cadre_Elipse(cfrmg)
c_FrMg	*cfrmg;
{
 Display	*display;
 Window		window;
...
void redraw_Create_Cadre_Elipse(cfrmg)
c_FrMg	*cfrmg;
{
 Display	*display;
 Window		window;
...
void Create_Cadre_Elipse_Done(cfrmg)
c_FrMg	*cfrmg;
{
 Display	*display;
 Window		window;
...
static void calcul_elipse(cfrmg)
c_FrMg	*cfrmg;
{ 
 int	dx, dy;
 DATA(Create_Cadre_Elipse);
...
void init_Create_Cadre_Polygonal(cfrmg)
c_FrMg	*cfrmg;
{
 ...
 F(GlobColormap).use_color(GlobColormap, cfrmg->X_info.gc_contour, BLACK);
 d(lcadre) = F(cfrmg).get_new_cadre(cfrmg);
 if (!d(lcadre))
 {
  Xc_WARNING(("Can't create cadre for an unknow reason"));
 }
 ...
 d(cadre)->thickness = SCALE_FROM_POINTS(.5);
...
status_t eventlook_Create_Cadre_Polygonal(cfrmg, event)
c_FrMg	*cfrmg;
XEvent	event;
{
 int		xm, ym;
 ...
 case ButtonPress:
  ...
  case 1:
   ...
   if (d(cont))
   {
    ...
    else
    {
     ...
     d(xo) = xm;
     d(yo) = ym;
     d(sxo) = d(xo) * cfrmg->sScale;
     d(syo) = d(yo) * cfrmg->sScale;
     F(cfrmg).attract_point(cfrmg, &d(sxo), &d(syo), &d(xo), &d(yo));
     F(d(cadre)).addspoint(d(cadre), 
			   d(sxo)-px * cfrmg->sScale, 
			   d(syo)-py * cfrmg->sScale, TRUE);
     d(xo) = d(sxo) / cfrmg->sScale;
     d(yo) = d(syo) / cfrmg->sScale;
   ...
   else
   {
    ...
    {
     d(prems) = FALSE;
     Create_Cadre_Polygonal_Done(cfrmg);
     F(CURSOR).pop_cursor(CURSOR);
     FREE_DATA;
  ...
  case 3:
   ...
   {
    F(cfrmg).delete_cadre(cfrmg, d(lcadre), DEL_CADRE);
    FREE_DATA;
...
void abort_Create_Cadre_Polygonal(cfrmg)
c_FrMg	*cfrmg;
{
 Display	*display;
 Window		window;
...
void redraw_Create_Cadre_Polygonal(cfrmg)
c_FrMg	*cfrmg;
{
 Display	*display;
 Window		window;
...
void Create_Cadre_Polygonal_Done(cfrmg)
c_FrMg	*cfrmg;
{
 Display	*display;
 Window		window;
 ...
 F(d(cadre)).set_X_info(d(cadre), cfrmg->X_info.display, 
			cfrmg->X_info.window);
 ...
 cfrmg->modify_cadre_callback(cfrmg->callback_data, 
			      ((d(cadre)->textflow_mode & XcTFM_HOLE_MASK) > 
			       XcTFM_OVER));
 
 F(cfrmg).draw_cadre(cfrmg, d(lcadre));
 F(cfrmg).display_selected_lcadre(cfrmg);
 F(cfrmg).select_cadre(cfrmg, d(lcadre), TRUE);
 cfrmg->verrou = FALSE;
}
