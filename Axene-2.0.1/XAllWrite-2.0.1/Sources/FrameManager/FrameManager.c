/*
** FrameManager.c for XAllWrite in FrameManager/
**
...
** Last update Sat Nov 14 19:19:48 1998 Emmanuel Paris
*/
...
static void redraw_rectangle_frmg();
static void redraw_polygone_frmg();
...
static boolean get_bbox_list_cadre ___PROTO((c_FrMg *This, bbox_t *bboxp,
					     l_Cadre *lcadre));
static void set_textflow_mode ___PROTO((c_FrMg *This, textflow_t mode));
...
int	num_function;
{
 ...
 case CUT_CADRE:
  cut_selected_cadre(This);
  ret_code = TRUE;
  break;
 case COPY_CADRE:
  copy_selected_cadre(This);
  ret_code = TRUE;
  break;
 case PASTE_CADRE:
  paste_selected_cadre(This);
  ret_code = TRUE;
  break;

 case TEXTFLOW_AROUND:
  set_textflow_mode(This, XcTFM_AROUND);
  ret_code = TRUE;
  break;
 case TEXTFLOW_LEFT:
  set_textflow_mode(This, XcTFM_LEFT);
  ret_code = TRUE;
  break;
 case TEXTFLOW_RIGHT:
  set_textflow_mode(This, XcTFM_RIGHT);
  ret_code = TRUE;
  break;
 case TEXTFLOW_SKIP:
  set_textflow_mode(This, XcTFM_SKIP);
  ret_code = TRUE;
  break;
 case TEXTFLOW_UNDER:
  set_textflow_mode(This, XcTFM_UNDER);
  ret_code = TRUE;
  break;
 case TEXTFLOW_OVER:
  set_textflow_mode(This, XcTFM_OVER);
  ret_code = TRUE;
  break;
 case TEXTFLOW_HOLE:
  set_textflow_mode(This, XcTFM_IN_HOLE);
  ret_code = TRUE;
  break;
 ...
 default:  
       ...
       num_function < VECTOR_FUNCTION_END))
  {
   ...
   if (!This->SelectedCadre || This->SelectedCadre->NextCadre ||
	...
	num_function < VECTOR_FUNCTION_END && 
	This->SelectedCadre->cadre->object_type != CADRE_VECTOR))
    ret_code = SetFunctionObject(This, FRMG_INVALID_FUNC);
   else
    ret_code = SetFunctionObject(This, num_function);

   if (This->function_data != NULL)
   {
...
boolean	delcadre;
{
 ...
 if ((this_cadre->textflow_mode & XcTFM_HOLE_MASK) > XcTFM_OVER)
  recalibrate = TRUE;
 
 delete_Cadre(This, lcadre, delcadre);

 This->modify_cadre_callback(This->callback_data, recalibrate);
 ...
 if (count_cadre)
 {
  ...
  for(i = count_cadre - 1; i >= 0; i--)
  {
   ...
   if ((tmplist->cadre->textflow_mode & XcTFM_HOLE_MASK) == XcTFM_OVER)
    This->redraw_polygone_callback(This->callback_data, 
				   lowcadre[i]->Forme, FALSE);
 ...
 if (tmplist)
 {
  do
  {
   ...
   if ((tmplist->cadre->textflow_mode & XcTFM_HOLE_MASK) == XcTFM_OVER)
    This->redraw_polygone_callback(This->callback_data, ftransp, FALSE);   
 ...
 if (!recalibrate)
  This->redraw_polygone_callback(This->callback_data, forme, TRUE);
...
/* ----------------------------------------------------------------- ** 
** get_clip_polygone: calcul clip mask for a rectangle area.         ** 
** ----------------------------------------------------------------- */
static void get_clip_polygone(This, x, y, width, height, clip_polygone,
...
{
 l_Cadre *lcadre, *tmplist, *lowlist, *highlist;
 c_Cadre *cadre;
 Polygone *forme, *ovforme, *unforme, *tmpform;
 
 forme = ovforme = unforme = NULL;
 lcadre = This->ListCadre;
 while(lcadre)
 {
  cadre = lcadre->cadre;

  if ((cadre->textflow_mode & XcTFM_HOLE_MASK) == XcTFM_UNDER)
  {
   if (IsRectIntersPolygone(cadre->Forme, x, y, width, height))
   {
    if (cadre->inside_color->transparency)
    {
     tmpform = DuplicatePolygone(cadre->Forme);

     list_cadre_low_high_priority(This, lcadre, 
				  &lowlist, &highlist, INTERS_CADRE);
     
     tmplist = lowlist;
     while(tmplist)
     {
      if (tmplist->cadre->inside_color->transparency && 
	  (tmplist->cadre->textflow_mode & XcTFM_HOLE_MASK)> XcTFM_OVER)
       SubtractPolygone(&tmpform, tmplist->cadre->Forme);
      tmplist = tmplist->NextCadre;
     }
     tmplist = highlist;
     while(tmplist)
     ...
     if (ovforme)
     {
      UnionPolygone(&ovforme, tmpform);
      FreePolygone(tmpform, DEL_POLYGONE);
     }
     else
      ovforme = tmpform; 
    }
  ...
  else if ((cadre->textflow_mode & XcTFM_HOLE_MASK) == XcTFM_OVER)
  {
   if (IsRectIntersPolygone(cadre->Forme, x, y, width, height))
   {
    tmpform = DuplicatePolygone(cadre->Forme);
 ...
 {
  *clip_polygone = SubtractRectPolygone(forme, x, y, width, height);
  FreePolygone(forme, DEL_POLYGONE);
 }

 *over_polygone = ovforme;
 *under_polygone = unforme;
...
l_Cadre	*lcadre;	
{
 ...
 if (this_cadre->inside_color->transparency)
 {
  ...
  if (count_cadre)
  {
   ...
   for(i = count_cadre - 1; i >= 0; i--)
   {
    F(tmp_list->cadre).drawcadre(tmp_list->cadre, lowcadre[i]->Forme);
    F(tmp_list->cadre).drawcontour(tmp_list->cadre, lowcadre[i]->Forme,
				   This->Frame_Border_actif);

    if ((tmp_list->cadre->textflow_mode & XcTFM_HOLE_MASK) == XcTFM_OVER)
     This->redraw_polygone_callback(This->callback_data, 
				    lowcadre[i]->Forme, FALSE);
 ...
 F(this_cadre).drawcontour(this_cadre,highforme, This->Frame_Border_actif);
 
 if ((this_cadre->textflow_mode & XcTFM_HOLE_MASK) == XcTFM_OVER)
  This->redraw_polygone_callback(This->callback_data, highforme, FALSE);
...
static void redraw_rectangle_frmg(This, x1, y1, x2, y2)
c_FrMg	*This;
int x1;
int y1;
int x2;
int y2;
{
 ...
 for(i = 0; i < This->nb_cadre; i++)
 {
  ...
  if (forme->memory)
  {
   ...
   if ((ListCadre->cadre->textflow_mode & XcTFM_HOLE_MASK) == XcTFM_OVER)
    This->redraw_polygone_callback(This->callback_data, forme, FALSE);
  }
  FreePolygone(forme, DEL_POLYGONE);
...
static void redraw_polygone_frmg(This, polygone, redraw)
c_FrMg	*This;
Polygone *polygone;
boolean redraw;
{
 ...
 {
  if (!redraw && 
      (ListCadre->cadre->textflow_mode & XcTFM_HOLE_MASK) == XcTFM_OVER)
  {
   ListCadre = ListCadre->NextCadre;
   continue;
  }
  
  forme = IntersectPolygone(ListCadre->cadre->Forme, polygone);
  if (forme->memory)
  {
   tmpcadre = ListCadre;
   for(j = i + 1; j < This->nb_cadre; j++)
...
unsigned short	h;
{
 ...
 if (This->select_type || !This->select_group)
 {
  ...
  if (This->SelectedCadre && !This->SelectedCadre->cadre->in_work)
  {
   ...
   {
    tmpcadre = This->SelectedCadre;
    while(tmpcadre && !tmpcadre->cadre->in_work)
     tmpcadre = tmpcadre->NextCadre;
    if (!tmpcadre)
     draw_handle(This);
   }
...
** set_textflow_mode                                                 ** 
** ----------------------------------------------------------------- */
static void set_textflow_mode(This, mode)
c_FrMg *This;
textflow_t mode;
{
 l_Cadre	*lcadre = This->SelectedCadre;

 while(lcadre)
 {
  if (mode == XcTFM_IN_HOLE)
   lcadre->cadre->textflow_mode ^= XcTFM_IN_HOLE;
  else
   lcadre->cadre->textflow_mode = 
    (lcadre->cadre->textflow_mode & ~XcTFM_HOLE_MASK) | mode;
  
  lcadre = lcadre->NextCadre;
 }
 
 if (!((mode & XcTFM_HOLE_MASK) > XcTFM_OVER))
  This->modify_cadre_callback(This->callback_data, FALSE);
 This->modify_cadre_callback(This->callback_data, TRUE);
}
...
c_Cadre	*cadre;
{
 ...
 lcadre = get_lcadre_by_cadre(cfrmg, cadre);
 this_cadre = lcadre->cadre;
 list_cadre_low_high_priority(cfrmg, lcadre, &lowlist, &highlist, 
			      INTERS_CADRE);
 tmp_list = highlist;
 forme = DuplicatePolygone(this_cadre->Forme);
 while(tmp_list)
