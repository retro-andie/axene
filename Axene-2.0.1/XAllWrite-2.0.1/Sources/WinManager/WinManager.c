/*
** WinManager.c for XAllWrite in WinManager/
**
...
** Last update Fri Jan  7 00:11:56 2000 Emmanuel Paris
*/
...
Pixmap		pixmap;
{
 ...
 listegwm->Normal_x = x; 
 listegwm->Normal_y = y;
 listegwm->Normal_w = width + wmADDEDX; 
 listegwm->Normal_h = height + wmADDEDY;
...
listmanaged	*listegwm;
{
 ...
 listegwm->Normal_x = realx;
 listegwm->Normal_y = realy;
}

void GwmIconWindow(This, display, listegwm, wantPlace)
c_WinManager	*This;
Display		*display;
listmanaged	*listegwm;
boolean		wantPlace;
 ...
 if (nbrwindow)
 {
  ...
  while (num < This->nbrmanaged)
  {
   if (Ptr->Type == D_NORMAL)
   {
    ...
    Ptr->Normal_x = placeX;
    Ptr->Normal_y = placeY;
    Ptr->Normal_w = width + wmADDEDX;
    Ptr->Normal_h = height + wmADDEDY;
 ...
 while (1)
 {
  ...
  if (Event->xany.window == listegwm->Use->IconWin)
  {
   ...
   if (Event->type == ButtonPress)
   {
    ...
    F(This).IconWindow(This, display, listegwm, TRUE);
   }
   return INTERN_EVENT;
  }
  /************************/
...
void GwmResizeWindow(This, display, manWin, x, y, width, height)
c_WinManager	*This;
Display		*display;
listmanaged	*manWin;
int		x;
...
int		height;
{
 if (manWin->Type == D_ICONED)
 {
  F(This).IconWindow(This, display, manWin, FALSE);
  return;
 }

 if (manWin->Type == D_MAXIMIZED)
 {
  Window parent, none, *SubWin;
  int dummy;

  XQueryTree(display, manWin->Win, &none, &parent, &SubWin, &dummy);
  XGetGeometry(display, parent, &none,
	       &x, &y, &width, &height, &dummy, &dummy);
  XMoveResizeWindow(display, manWin->Win, 0, 0, width, height);
  XMoveResizeWindow(display, manWin->MainWin, 0, 0, width, height);
  XMoveWindow(display, manWin->Use->UnMaxWin, width - BUTTON_UMAXW, 0);
  XMapSubwindows(display, manWin->MainWin);
  XMapWindow(display, manWin->MainWin);
  XMapRaised(display, manWin->Use->UnMaxWin);
  XMapRaised(display, manWin->Win);
  return;
 }

 XMoveResizeWindow(display, manWin->Win, x, y, width, height);
 XUnmapWindow(display, manWin->Use->UnMaxWin);
 
 XMoveResizeWindow(display, manWin->MainWin,
		   RESIZE_WIDTH+1, BUTTON_SIZEY + 1,
		   width - wmADDEDX -2,
		   height - wmADDEDY);
 XMapWindow(display, manWin->MainWin);
 ...
 height -= wmADDEDY;
 width -= wmADDEDX;
 XResizeWindow(display, manWin->Use->TitleWin, wmtitleW, wmtitleH);
 XMoveWindow(display, manWin->Use->IconWin, wmiconX, wmiconY);
 XMoveWindow(display, manWin->Use->MaxWin, wmmaxX, wmmaxY);
 XResizeWindow(display, manWin->Use->ResizeLeftWin, wmresizelW, wmresizelH);
 XMoveResizeWindow(display, manWin->Use->ResizeRightWin,
		   wmresizerX, wmresizerY,
		   wmresizerW, wmresizerH);
 XMoveWindow(display, manWin->Use->ResizeCornerLeftWin,
	     wmresizecornerlX, wmresizecornerlY);
 XMoveWindow(display, manWin->Use->ResizeCornerRightWin, 
	     wmresizecornerrX, wmresizecornerrY);
 XMoveResizeWindow(display, manWin->Use->ResizeBottomWin,
		   wmresizebottomX, wmresizebottomY,
		   wmresizebottomW, wmresizebottomH);
 XResizeWindow(display, manWin->MainWin, wmmainW, wmmainH);

 XMapWindow(display, manWin->Use->CloseWin);
 XMapWindow(display, manWin->Use->TitleWin);
 XMapWindow(display, manWin->Use->MaxWin);
 XMapWindow(display, manWin->Use->IconWin);
 XMapWindow(display, manWin->Use->ResizeBottomWin);
 XMapWindow(display, manWin->Use->ResizeRightWin);
 XMapWindow(display, manWin->Use->ResizeLeftWin);
 XMapWindow(display, manWin->Use->ResizeCornerRightWin);
 XMapWindow(display, manWin->Use->ResizeCornerLeftWin);
 XMapSubwindows(display, manWin->MainWin);
 XMapWindow(display, manWin->MainWin);
 XMapRaised(display, manWin->Win);
}

void GwmUpdateWindow(This, display, mainwin)
c_WinManager	*This;
Display		*display;
Window		mainwin;
{
 ...
 XUnmapSubwindows(display, mainwin);

 ManagedTmp = D(This).ListManaged;
 while (ManagedTmp != NULL)
 {
  GwmResizeWindow(This, display, ManagedTmp, 
		  ManagedTmp->Normal_x, ManagedTmp->Normal_y,
		  ManagedTmp->Normal_w, ManagedTmp->Normal_h);
  GwmDrawDecorP(This, ManagedTmp, display, 0, 0, MGWMALL);
  ManagedTmp = ManagedTmp->NextManaged;
 }
 if (D(This).SelectedWindow->Type != D_ICONED)
 {
  XMapRaised(display, This->SelectedWindow->Win);
  GwmDrawDecorP(This, This->SelectedWindow, display, 0, 0, MGWMALL);
 }
}
