/*
** AutoScroll.c for XAllWrite in AutoScroll/
...
** Last update Sat Jan  3 16:39:22 1998 Emmanuel Paris
*/
...
XEvent	*event;
{
 ...
 case Expose:
  /*case NoExpose: */
  /*        I don't know why the event->xany.window is invalid in this case !*/
 case GraphicsExpose:
 case LeaveNotify:
  XQueryPointer(This->display, event->xany.window, &fr, &ff,
		&xr, &yr, &xm, &ym, &mdb);
  event->type = MotionNotify;
  event->xmotion.x_root = xr;
  event->xmotion.y_root = yr;
 case ButtonPress:
 case MotionNotify:
  This->x = event->xmotion.x_root;
  This->y = event->xmotion.y_root;
 ...
 if (ret)
 {
  ...
  do
  {
   ...
   if (!sortie)
   {
    ...
    Xc_TRACE(("x: %d y: %d [%d, %d]->[%d, %d]", This->x, This->y,
	   This->x_min, This->y_min, This->x_max, This->y_max));
...
XEvent	*event;
{
 ...
 xm = event->xmotion.x_root;
 ym = event->xmotion.y_root;
